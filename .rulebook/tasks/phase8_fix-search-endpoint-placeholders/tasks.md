...
- [ ] 1.1 Extract the core of `search_vectors_by_text`
...
- [ ] 1.2 Rewrite `search_vectors` (same file, line 351) to parse
...
- [ ] 1.3 Register the handler for `/collections/{name}/search` with a
...
- [ ] 1.4 Apply the query cache with `QueryKey::from_vector(vector,
  limit, threshold)` and verify the hit path returns from cache
  identically to the text handler.
- [ ] 1.5 Document response shape in a doc comment over the handler.
