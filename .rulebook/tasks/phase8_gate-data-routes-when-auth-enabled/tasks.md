...
- [ ] 1.1 Apply `require_auth_middleware` as a router layer on
  `rest_routes` in
...
- [ ] 1.2 Move `/auth/validate-password` into the public bucket
  (already routes through `public_auth_router` — confirm it is merged
  after the auth layer).
- [ ] 1.3 Verify the admin router still works under the new layer
  stack (admin routes must still 401 unauthenticated, 403 without
  `Role::Admin`).
- [ ] 1.4 Add a startup log line at INFO when auth enforcement is
  active so operators see the mode in the banner.
- [ ] 1.5 Confirm the dashboard SPA's own login flow still fetches
