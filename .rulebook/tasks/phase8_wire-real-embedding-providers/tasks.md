...
- [ ] 1.1 Add `FastEmbedProvider` adapter at
...
- [ ] 1.2 Expose `OnnxEmbedder::new_from_config(cfg)` and
...
- [ ] 1.3 Parse `config.embedding.model`, `default_model`, and the
...
- [ ] 1.4 Register the resolved provider on all three embedding
  managers the bootstrap creates (server, file watcher, final), not
  just the first.
- [ ] 1.5 Add boot-time validation: if the config names a backend
...
- [ ] 2.1 Update or create documentation covering the implementation
  ...
  `3.0.0 > Added`).
- [ ] 2.2 Write tests covering the new behavior (integration test
...
- [ ] 2.3 Run tests and confirm they pass (the new integration test
