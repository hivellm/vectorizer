...
- [ ] 1.1 Extract the chunk+embed+store body of `insert_text` into a
...
- [ ] 1.2 Rewrite `batch_insert_texts` in `vectors.rs` to parse
...
- [ ] 1.3 Rewrite `insert_texts` the same way (same handler body —
  consider routing both endpoints to one function).
- [ ] 1.4 Validate payload shape: return 400 if `collection` missing,
  if `texts` missing, if `texts` empty, or if a per-item `text` is
  missing.
- [ ] 1.5 Propagate the real `METRICS.insert_latency_seconds` timer
