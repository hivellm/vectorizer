...
**Earlier in v3.x**: 3.6.0 added collection TTL with read-back and full RPC parity; 3.5.0 fixed BM25 vocabulary loss across restarts, made the WAL fsync and checksum its records, removed a `bulk_update_metadata` deadlock, and unblocked search during batch inserts; 3.4.0 made `/data` the canonical volume mount.
...
- **Base Image**: the default image is `scratch` (zero OS packages, no shell) as of 3.7.1. Earlier history: v3.0.0 shipped on `gcr.io/distroless/cc-debian12:nonroot`; v3.0.2 swapped to the Docker Hardened `dhi.io/debian-base:trixie` (pinned by digest since 3.5.0) to make Scout's "Approved Base Images" + "Up-to-Date Base Images" policies flip to `Compliant`. The `-fastembed` variant needs a full runtime (ONNX Runtime + libstdc++) and, since 3.8.2, runs on public `gcr.io/distroless/cc-debian13:nonroot` (glibc), also pinned by digest — no Docker Hub account is required to build or pull either variant.
...
From 3.8.2 the published `-fastembed` image bakes `multilingual-e5-small` (`intfloat/multilingual-e5-small`) into `/vectorizer/models/fastembed` and points the `VECTORIZER_FASTEMBED_CACHE_DIR` env var at it — outside `/data`, so pods never download it at boot and a volume mounted on `/data` (every Kubernetes PVC) cannot hide it. Other models still download to `$VECTORIZER_FASTEMBED_CACHE_DIR` (or `/data/fastembed` if that variable is unset) on first boot; keep that path on a volume so it happens once. To bake a different model into your own build, add `--build-arg FASTEMBED_MODEL=<Hugging Face repo>` — e.g. `--build-arg FASTEMBED_MODEL=intfloat/multilingual-e5-small` for `fastembed:multilingual-e5-small` (the repo per id is listed in `docs/specs/EMBEDDING.md`).
