...
use super::types::{CollectionRef, DiscoveryResult, ScoredChunk, ChunkMetadata, DiscoveryError};
...
pub fn sentence_quality_score(sentence: &str) -> f32 {
    ...
    if sentence.chars().next().map(|c| c.is_uppercase()).unwrap_or(false) {
        score += 0.1;
    }
    ...
    if indicators.iter().any(|i| sentence.to_lowercase().contains(i)) {
        score += 0.1;
    }
