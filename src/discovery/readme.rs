...
pub fn promote_readme(
    ...
    if config.always_top {
        ...
        readme_chunks.sort_by(|a, b| b.score.partial_cmp(&a.score).unwrap_or(std::cmp::Ordering::Equal));
        other_chunks.sort_by(|a, b| b.score.partial_cmp(&a.score).unwrap_or(std::cmp::Ordering::Equal));
...
fn is_readme(file_path: &str, config: &ReadmePromotionConfig) -> bool {
    ...
    config.readme_patterns.iter().any(|pattern| {
