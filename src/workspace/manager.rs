...
impl WorkspaceManager {
    ...
    pub fn get_project_path(&self, project_name: &str) -> Result<PathBuf> {
        ...
        Ok(self.workspace_root.join(&project.path))
    }
    ...
    pub fn add_project(&mut self, project: ProjectConfig) -> Result<()> {
        ...
        let project_path = self.workspace_root.join(&project.path);
...
mod tests {
    ...
    fn test_workspace_manager_new() {
        ...
        assert_eq!(manager.workspace_root, temp_dir.path().to_path_buf());
        assert_eq!(manager.config_path, config_path);
    }

    #[test]
    fn test_workspace_manager_getters() {
        let temp_dir = tempdir().unwrap();
        let config = WorkspaceConfig::default();
        let config_path = temp_dir.path().join("vectorize-workspace.yml");
    ...
    #[test]
    fn test_workspace_manager_with_projects() {
        let temp_dir = tempdir().unwrap();
        let mut manager = WorkspaceManager::create_default(temp_dir.path()).unwrap();

        // Add multiple projects
        let project1 = ProjectConfig {
            name: "project1".to_string(),
            path: PathBuf::from("project1"),
            description: "Test project 1".to_string(),
            enabled: true,
            embedding: None,
            collections: vec![],
        };

        let project2 = ProjectConfig {
            name: "project2".to_string(),
            path: PathBuf::from("project2"),
            description: "Test project 2".to_string(),
            enabled: false,
            embedding: None,
            collections: vec![],
        };
        ...
        std::fs::create_dir_all(temp_dir.path().join("project1")).unwrap();
        std::fs::create_dir_all(temp_dir.path().join("project2")).unwrap();

        manager.add_project(project1).unwrap();
        manager.add_project(project2).unwrap();

        // Test project retrieval
        assert!(manager.get_project("project1").is_some());
        assert!(manager.get_project("project2").is_some());
        assert!(manager.get_project("nonexistent").is_none());

        // Test project listing
        let projects = manager.enabled_projects();
        assert_eq!(projects.len(), 1); // Only project1 is enabled
        assert!(projects.iter().any(|p| p.name == "project1"));

        // Test status with projects
        let status = manager.get_status();
        assert_eq!(status.total_projects, 2);
        assert_eq!(status.enabled_projects, 1);
    }

    #[test]
    fn test_workspace_manager_update_project() {
        let temp_dir = tempdir().unwrap();
        let mut manager = WorkspaceManager::create_default(temp_dir.path()).unwrap();
        ...
        std::fs::create_dir_all(temp_dir.path().join("test-project")).unwrap();

        manager.add_project(project).unwrap();

        // Update project by removing and re-adding
        let mut updated_project = manager.get_project("test-project").unwrap().clone();
        updated_project.enabled = false;
        manager.remove_project("test-project").unwrap();
        manager.add_project(updated_project).unwrap();

        let retrieved_project = manager.get_project("test-project").unwrap();
        assert!(!retrieved_project.enabled);
    }

    #[test]
    fn test_workspace_manager_update_nonexistent_project() {
        let temp_dir = tempdir().unwrap();
        let mut manager = WorkspaceManager::create_default(temp_dir.path()).unwrap();
        ...
        let result = manager.add_project(project);
        assert!(result.is_err());
    }

    #[test]
    fn test_workspace_manager_remove_nonexistent_project() {
        let temp_dir = tempdir().unwrap();
        let mut manager = WorkspaceManager::create_default(temp_dir.path()).unwrap();
    ...
    #[test]
    fn test_workspace_manager_duplicate_project() {
        let temp_dir = tempdir().unwrap();
        let mut manager = WorkspaceManager::create_default(temp_dir.path()).unwrap();
    ...
    #[test]
    fn test_workspace_manager_with_collections() {
        let temp_dir = tempdir().unwrap();
        let mut manager = WorkspaceManager::create_default(temp_dir.path()).unwrap();
        ...
        let collection = CollectionConfig {
            ...
            dimension: 128,
            metric: crate::workspace::config::DistanceMetric::Cosine,
            ..Default::default()
        };
    ...
    fn test_workspace_manager_save_and_load() {
        ...
        let loaded_manager = WorkspaceManager::load_from_file(manager.config_path()).unwrap();
        assert!(loaded_manager.get_project("test-project").is_some());
    }

    #[test]
    fn test_workspace_manager_invalid_config_path() {
        let result = WorkspaceManager::load_from_file("/nonexistent/path/config.yml");
        assert!(result.is_err());
    }

    #[test]
    fn test_workspace_manager_create_default_invalid_path() {
        let result = WorkspaceManager::create_default("/nonexistent/path");
        // Accept current behavior - may create directory or fail
