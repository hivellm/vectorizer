...
impl WorkspaceManager {
    ...
    pub fn load_from_file<P: AsRef<Path>>(config_path: P) -> Result<Self> {
        ...
        info!("Parsed workspace config with {} projects from YAML", config.projects.len());
    ...
    pub fn find_and_load<P: AsRef<Path>>(start_path: P) -> Result<Self> {
        ...
        info!("Searching for workspace configuration starting from: {}", start_path.display());
        
        // Find configuration file
        let config_path = find_workspace_config(start_path)?
            .ok_or_else(|| VectorizerError::IoError(std::io::Error::new(
    ...
    pub fn create_default<P: AsRef<Path>>(workspace_root: P) -> Result<Self> {
        ...
        info!("Creating default workspace at: {}", workspace_root.display());
        
        // Create default configuration
        let mut config = create_default_workspace_config();
        
        // Update workspace metadata
        config.workspace.name = workspace_root.file_name()
    ...
    pub fn enabled_projects(&self) -> Vec<&ProjectConfig> {
        ...
        let enabled = self.config.projects.iter().filter(|p| p.enabled).collect::<Vec<_>>();
        tracing::info!("Workspace has {} total projects, {} enabled", total_projects, enabled.len());
        enabled
    }
    ...
    pub fn get_collection_config(&self, project_name: &str, collection_name: &str) -> Result<&CollectionConfig> {
        let project = self.get_project(project_name)
            .ok_or_else(|| VectorizerError::IoError(std::io::Error::new(
                std::io::ErrorKind::NotFound,
                format!("Project '{}' not found", project_name)
        ...
        project.collections.iter()
            .find(|c| c.name == collection_name)
            .ok_or_else(|| VectorizerError::IoError(std::io::Error::new(
                std::io::ErrorKind::NotFound,
                format!("Collection '{}' not found in project '{}'", collection_name, project_name)
            )))
    }
    ...
    pub fn add_project(&mut self, project: ProjectConfig) -> Result<()> {
        ...
        info!("Added project: {}", self.config.projects.last().unwrap().name);
        Ok(())
    }
    ...
    pub fn set_project_enabled(&mut self, project_name: &str, enabled: bool) -> Result<()> {
        ...
        let project = self.config.projects.iter_mut()
    ...
    pub fn add_collection(&mut self, project_name: &str, collection: CollectionConfig) -> Result<()> {
        debug!("Adding collection '{}' to project '{}'", collection.name, project_name);
        ...
        if project.collections.iter().any(|c| c.name == collection.name) {
            return Err(VectorizerError::IoError(std::io::Error::new(
                std::io::ErrorKind::AlreadyExists,
                format!("Collection '{}' already exists in project '{}'", collection.name, project_name)
            )));
        }
        ...
        info!("Added collection '{}' to project '{}'", project.collections.last().unwrap().name, project_name);
        Ok(())
    }
    
    /// Remove a collection from a project
    pub fn remove_collection(&mut self, project_name: &str, collection_name: &str) -> Result<()> {
        debug!("Removing collection '{}' from project '{}'", collection_name, project_name);
        ...
        info!("Removed collection '{}' from project '{}'", collection_name, project_name);
        Ok(())
    }
    
    /// Save workspace configuration
    pub fn save(&self) -> Result<()> {
        debug!("Saving workspace configuration to: {}", self.config_path.display());
    ...
    pub fn reload(&mut self) -> Result<()> {
        info!("Reloading workspace configuration from: {}", self.config_path.display());
    ...
    pub fn get_status(&self) -> WorkspaceStatus {
        ...
        let total_collections: usize = enabled_projects.iter()
