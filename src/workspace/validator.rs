...
pub fn validate_workspace_config(config: &WorkspaceConfig, workspace_root: &Path) -> ValidationResult {
    ...
    if result.is_valid {
        ...
        error!("Workspace configuration validation failed with {} errors", result.errors.len());
    }
    
    if !result.warnings.is_empty() {
        warn!("Workspace configuration validation completed with {} warnings", result.warnings.len());
    }
...
fn validate_global_settings(config: &WorkspaceConfig, result: &mut ValidationResult) {
    ...
    validate_embedding_config(&config.global.default_embedding, "global.default_embedding", result);
...
/// Validate projects
fn validate_projects(config: &WorkspaceConfig, workspace_root: &Path, result: &mut ValidationResult) {
    ...
    for (index, project) in config.projects.iter().enumerate() {
        ...
        if !project_names.insert(&project.name) {
            result.add_error(format!("{}: Duplicate project name '{}'", project_prefix, project.name));
        }
...
fn validate_project_config(project: &ProjectConfig, workspace_root: &Path, prefix: &str, result: &mut ValidationResult) {
    ...
    if !project_path.exists() {
        result.add_error(format!("{}: Project path does not exist: {}", prefix, project_path.display()));
    } else if !project_path.is_dir() {
        result.add_error(format!("{}: Project path is not a directory: {}", prefix, project_path.display()));
    }
...
/// Validate collections
fn validate_collections(collections: &[CollectionConfig], prefix: &str, result: &mut ValidationResult) {
    ...
    for (index, collection) in collections.iter().enumerate() {
        ...
        if !collection_names.insert(&collection.name) {
            result.add_error(format!("{}: Duplicate collection name '{}'", collection_prefix, collection.name));
        }
...
fn validate_collection_config(collection: &CollectionConfig, prefix: &str, result: &mut ValidationResult) {
    ...
    if collection.dimension == 0 {
        result.add_error(format!("{}: Collection dimension must be greater than 0", prefix));
    } else if collection.dimension > 4096 {
        result.add_warning(format!("{}: Collection dimension {} is very large, consider using a smaller dimension", prefix, collection.dimension));
    }
    
    // Validate embedding configuration
    validate_embedding_config(&collection.embedding, &format!("{}.embedding", prefix), result);
    
    // Validate indexing configuration
    validate_indexing_config(&collection.indexing, &format!("{}.indexing", prefix), result);
...
/// Validate embedding configuration
fn validate_embedding_config(embedding: &EmbeddingConfig, prefix: &str, result: &mut ValidationResult) {
    debug!("Validating embedding configuration: {}", prefix);
    
    // Validate dimension
    if embedding.dimension == 0 {
        result.add_error(format!("{}: Embedding dimension must be greater than 0", prefix));
    }
...
fn validate_bow_parameters(parameters: &std::collections::HashMap<String, serde_json::Value>, prefix: &str, result: &mut ValidationResult) {
    ...
    if let Some(max_seq_len) = parameters.get("max_sequence_length") {
        if let Some(len) = max_seq_len.as_u64() {
            if len == 0 {
                result.add_error(format!("{}: max_sequence_length must be greater than 0", prefix));
            }
        }
    }
}
...
fn validate_hash_parameters(parameters: &std::collections::HashMap<String, serde_json::Value>, prefix: &str, result: &mut ValidationResult) {
...
fn validate_ngram_parameters(parameters: &std::collections::HashMap<String, serde_json::Value>, prefix: &str, result: &mut ValidationResult) {
    if let Some(ngram_range) = parameters.get("ngram_range") {
        if let Some(range) = ngram_range.as_array() {
            if range.len() != 2 {
                result.add_error(format!("{}: ngram_range must have exactly 2 elements", prefix));
            } else {
                if let (Some(min), Some(max)) = (range[0].as_u64(), range[1].as_u64()) {
                    if min > max {
                        result.add_error(format!("{}: ngram_range min ({}) must be <= max ({})", prefix, min, max));
                    }
                }
            }
        }
...
fn validate_bm25_parameters(parameters: &std::collections::HashMap<String, serde_json::Value>, prefix: &str, result: &mut ValidationResult) {
    ...
    if let Some(b) = parameters.get("b") {
        if let Some(b_val) = b.as_f64() {
            if b_val < 0.0 || b_val > 1.0 {
                result.add_error(format!("{}: BM25 b parameter must be between 0 and 1", prefix));
            }
        }
    }
}
...
fn validate_real_model_parameters(parameters: &std::collections::HashMap<String, serde_json::Value>, prefix: &str, result: &mut ValidationResult) {
...
fn validate_onnx_model_parameters(parameters: &std::collections::HashMap<String, serde_json::Value>, prefix: &str, result: &mut ValidationResult) {
    if let Some(model_type) = parameters.get("model_type") {
        if let Some(model_str) = model_type.as_str() {
            if model_str.is_empty() {
                result.add_error(format!("{}: model_type cannot be empty", prefix));
...
/// Validate indexing configuration
fn validate_indexing_config(indexing: &IndexingConfig, prefix: &str, result: &mut ValidationResult) {
...
/// Validate HNSW parameters
fn validate_hnsw_parameters(parameters: &std::collections::HashMap<String, serde_json::Value>, prefix: &str, result: &mut ValidationResult) {
    if let Some(m) = parameters.get("m") {
        if let Some(m_val) = m.as_u64() {
            if m_val == 0 {
                result.add_error(format!("{}: HNSW m parameter must be greater than 0", prefix));
            }
        }
    }
    ...
    if let Some(ef_construction) = parameters.get("ef_construction") {
        if let Some(ef_val) = ef_construction.as_u64() {
            if ef_val == 0 {
                result.add_error(format!("{}: HNSW ef_construction must be greater than 0", prefix));
            }
        }
    }
...
fn validate_collection_processing(processing: &CollectionProcessing, prefix: &str, result: &mut ValidationResult) {
    ...
    if processing.chunk_overlap >= processing.chunk_size {
        result.add_error(format!("{}: chunk_overlap must be less than chunk_size", prefix));
    }
    
    if processing.include_patterns.is_empty() {
        result.add_warning(format!("{}: No include patterns specified, all files will be processed", prefix));
    }
}
