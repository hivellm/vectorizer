...
mod tests {
    ...
    #[test]
    fn test_intelligent_search_config_default() {
        let config = IntelligentSearchConfig::default();
        assert_eq!(config.max_queries, 8);
        assert!(config.domain_expansion);
        assert!(config.technical_focus);
    ...
    #[test]
    fn test_intelligent_search_config_custom() {
    ...
    fn test_search_result_creation() {
        let result = crate::evaluation::QueryResult {
            doc_id: "result1".to_string(),
            relevance: 0.95,
        };

        assert_eq!(result.doc_id, "result1");
    ...
    fn test_search_result_with_metadata() {
        ...
        let result = crate::evaluation::QueryResult {
            doc_id: "result2".to_string(),
            relevance: 0.87,
        };

        assert_eq!(result.doc_id, "result2");
        assert_eq!(result.relevance, 0.87);
    }
    ...
    fn test_document_with_metadata() {
        ...
        let doc = Document {
            id: "doc2".to_string(),
            content: "This is another test document".to_string(),
            collection: "test_collection".to_string(),
            metadata,
        };

        assert_eq!(doc.id, "doc2");
        assert_eq!(doc.content, "This is another test document");
    ...
    #[test]
    fn test_intelligent_search_config_serialization() {
        let config = IntelligentSearchConfig::default();
        let serialized = serde_json::to_string(&config).unwrap();
        let deserialized: IntelligentSearchConfig = serde_json::from_str(&serialized).unwrap();

        assert_eq!(config.max_queries, deserialized.max_queries);
        assert_eq!(config.domain_expansion, deserialized.domain_expansion);
        assert_eq!(config.technical_focus, deserialized.technical_focus);
        assert_eq!(config.synonym_expansion, deserialized.synonym_expansion);
        assert_eq!(
            config.similarity_threshold,
            deserialized.similarity_threshold
        );
        assert_eq!(config.reranking_enabled, deserialized.reranking_enabled);
        assert_eq!(config.mmr_enabled, deserialized.mmr_enabled);
        assert_eq!(config.mmr_lambda, deserialized.mmr_lambda);
    }
    ...
    fn test_search_result_serialization() {
        ...
        let result = crate::evaluation::QueryResult {
            doc_id: "test_id".to_string(),
            relevance: 0.5,
        };

        let serialized = serde_json::to_string(&result).unwrap();
        let deserialized: crate::evaluation::QueryResult =
            serde_json::from_str(&serialized).unwrap();

        assert_eq!(result.doc_id, deserialized.doc_id);
        assert_eq!(result.relevance, deserialized.relevance);
    }
}
