...
impl MCPToolHandler {
    ...
    pub fn new(store: std::sync::Arc<crate::VectorStore>, embedding_manager: std::sync::Arc<crate::embedding::EmbeddingManager>) -> Self {
        Self {
            store,
            embedding_manager,
        }
    ...
    pub fn new_with_store(store: std::sync::Arc<crate::VectorStore>) -> Self {
        ...
        placeholder_manager.set_default_provider("bm25").unwrap_or_default();
    ...
    pub async fn handle_intelligent_search(
                            ...
                            Ok(search_results) => {
                                for result in search_results {
                                    let intelligent_result = IntelligentSearchResult {
                                        ...
                                        score: result.score,
                                        metadata: result.payload.as_ref()
                                            .map(|p| p.data.as_object().unwrap().clone().into_iter().collect::<HashMap<String, serde_json::Value>>())
                                            .unwrap_or_default(),
                                        score_breakdown: Some(ScoreBreakdown {
                                            relevance: result.score,
                                            collection_bonus: if collection.contains("cmmv") { 0.1 } else { 0.0 },
                                            technical_bonus: if query.contains("api") || query.contains("framework") { 0.1 } else { 0.0 },
        ...
        let final_results = if tool.mmr_enabled.unwrap_or(true) {
            self.apply_mmr_diversification(&deduped_results, max_results, tool.mmr_lambda.unwrap_or(0.7))
        } else {
            deduped_results.clone().into_iter().take(max_results).collect()
        };
        ...
        tool_metadata.insert("tool_name".to_string(), serde_json::Value::String("intelligent_search".to_string()));
    ...
    pub async fn handle_multi_collection_search(
        ...
        let final_results: Vec<IntelligentSearchResult> = final_results.into_iter().take(max_total_results).collect();
        ...
        let mut tool_metadata = HashMap::new();
        tool_metadata.insert("tool_name".to_string(), serde_json::Value::String("multi_collection_search".to_string()));
        tool_metadata.insert("collections_searched".to_string(), serde_json::Value::Number(serde_json::Number::from(tool.collections.len())));
        tool_metadata.insert("cross_collection_reranking".to_string(), serde_json::Value::Bool(tool.cross_collection_reranking.unwrap_or(true)));
        
        for (collection, count) in &collection_results {
            tool_metadata.insert(format!("results_{}", collection), serde_json::Value::Number(serde_json::Number::from(*count)));
        }
    ...
    pub async fn handle_semantic_search(
                        ...
                        Ok(search_results) => {
                            for result in search_results {
                                let intelligent_result = IntelligentSearchResult {
                                    ...
                                    score: result.score,
                                    metadata: result.payload.as_ref()
                                        .map(|p| p.data.as_object().unwrap().clone().into_iter().collect())
                                        .unwrap_or_default(),
                                    score_breakdown: Some(ScoreBreakdown {
                                        relevance: result.score,
                                        collection_bonus: 0.0,
        ...
        let mut tool_metadata = HashMap::new();
        tool_metadata.insert("tool_name".to_string(), serde_json::Value::String("semantic_search".to_string()));
        tool_metadata.insert("semantic_reranking".to_string(), serde_json::Value::Bool(tool.semantic_reranking.unwrap_or(true)));
        tool_metadata.insert("cross_encoder_reranking".to_string(), serde_json::Value::Bool(tool.cross_encoder_reranking.unwrap_or(false)));
        tool_metadata.insert("similarity_threshold".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(similarity_threshold as f64).unwrap()));
    ...
    pub async fn handle_contextual_search(
        ...
        let mut tool_metadata = HashMap::new();
        tool_metadata.insert("tool_name".to_string(), serde_json::Value::String("contextual_search".to_string()));
        tool_metadata.insert("context_filters_applied".to_string(), serde_json::Value::Bool(tool.context_filters.is_some()));
        tool_metadata.insert("context_reranking".to_string(), serde_json::Value::Bool(tool.context_reranking.unwrap_or(true)));
        tool_metadata.insert("context_weight".to_string(), serde_json::Value::Number(serde_json::Number::from_f64(tool.context_weight.unwrap_or(0.3) as f64).unwrap()));
    ...
    fn apply_mmr_diversification(&self, results: &[IntelligentSearchResult], max_results: usize, lambda: f32) -> Vec<IntelligentSearchResult> {
        ...
        if let Some(first) = remaining.iter().max_by(|a, b| a.score.partial_cmp(&b.score).unwrap()) {
            let first_idx = remaining.iter().position(|r| r.doc_id == first.doc_id).unwrap();
        ...
        while selected.len() < max_results && !remaining.is_empty() {
            ...
            for (idx, candidate) in remaining.iter().enumerate() {
                ...
                let max_similarity = selected.iter()
                    .map(|selected| self.calculate_content_similarity(&candidate.content, &selected.content))
                    .fold(0.0, f32::max);
    ...
    /// Semantic reranking
    fn semantic_rerank(&self, mut results: Vec<IntelligentSearchResult>, _query: &str) -> Vec<IntelligentSearchResult> {
        // Simple implementation: boost results with technical terms
        for result in &mut results {
            if result.content.contains("API") || result.content.contains("framework") || result.content.contains("implementation") {
                result.score *= 1.1;
            }
        }
    ...
    fn cross_encoder_rerank(&self, mut results: Vec<IntelligentSearchResult>, _query: &str) -> Vec<IntelligentSearchResult> {
        // Simple implementation: boost results with exact matches
        for result in &mut results {
            if result.content.to_lowercase().contains(_query.to_lowercase().as_str()) {
                result.score *= 1.2;
            }
        }
    ...
    /// Filter by context
    fn filter_by_context(&self, results: Vec<IntelligentSearchResult>, _context_filters: &HashMap<String, serde_json::Value>) -> Vec<IntelligentSearchResult> {
    ...
    fn context_aware_rerank(&self, mut results: Vec<IntelligentSearchResult>, _query: &str, _context_weight: f32) -> Vec<IntelligentSearchResult> {
