...
impl MCPServerIntegration {
            ...
            Self::create_tool_schema(
                ...
                vec![
                    ...
                    (
                        ...
                        "number",
                        false,
                        "Context weight in scoring (0.0-1.0)",
                    ),
                ],
            ),
        ]
    ...
    fn create_tool_schema(
        ...
        for (param_name, param_type, is_required, param_description) in parameters {
            ...
            param_schema.insert(
                "description".to_string(),
                Value::String(param_description.to_string()),
            );
    ...
    pub async fn handle_rest_request(
        ...
        match endpoint {
            "/api/intelligent-search" => {
                ...
                self.rest_handler
                    .validate_intelligent_search_request(&request)?;
                let response = self
                    .rest_handler
            ...
            "/api/multi-collection-search" => {
                ...
                self.rest_handler
                    .validate_multi_collection_search_request(&request)?;
                let response = self
                    .rest_handler
            ...
            "/api/semantic-search" => {
                ...
                self.rest_handler
                    .validate_semantic_search_request(&request)?;
                let response = self
                    .rest_handler
            ...
            "/api/contextual-search" => {
                ...
                self.rest_handler
                    .validate_contextual_search_request(&request)?;
                let response = self
                    .rest_handler
    ...
    pub fn get_rest_endpoints(&self) -> Vec<HashMap<String, Value>> {
        vec![
            ...
            Self::create_endpoint_schema("/api/docs", "GET", "Get API documentation"),
        ]
    }

    /// Create endpoint schema
    fn create_endpoint_schema(
        path: &str,
        method: &str,
        description: &str,
    ) -> HashMap<String, Value> {
...
mod tests {
    ...
    fn test_create_tool_schema() {
        ...
        assert_eq!(
            schema["description"],
            Value::String("Test tool description".to_string())
        );
