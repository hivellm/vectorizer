...
impl CacheManager {
    ...
    pub async fn remove_collection_info(&self, name: &str) -> CacheResult<Option<CollectionCacheInfo>> {
        let result = {
            let mut metadata = self.metadata.write().await;
    ...
    pub async fn has_collection(&self, name: &str) -> bool {
        ...
        info!("🔍 CacheManager::has_collection result for '{}': {}", name, result);
        result
    }
    ...
    async fn validate_basic(&self) -> CacheResult<ValidationResult> {
        ...
        if !self.metadata_path.exists() {
            ...
            result.errors.push("Metadata file does not exist".to_string());
            return Ok(result);
        }
        ...
        if !self.cache_path.exists() {
            result.status = ValidationStatus::Invalid;
            result.errors.push("Cache directory does not exist".to_string());
            return Ok(result);
        }
        ...
        for (name, collection_info) in &metadata.collections {
            if collection_info.name != *name {
                result.errors.push(format!("Collection name mismatch: {} != {}", name, collection_info.name));
            }
        }
            ...
            for (file_path, file_info) in &collection_info.file_hashes {
                if !file_path.exists() {
                    result.errors.push(format!(
                        ...
                        file_path.display(), collection_name
                    ));
                } else {
                    // Check file size
                    if let Ok(metadata) = std::fs::metadata(file_path) {
                        if metadata.len() != file_info.size {
                            result.errors.push(format!(
                                ...
                                file_path.display(), file_info.size, metadata.len()
                            ));
                        }
                    }
                }
    ...
    pub fn summary(&self) -> String {
        match self.status {
            ...
            ValidationStatus::ValidWithWarnings => {
                format!("Cache validation passed with {} warnings", self.warnings.len())
            }
