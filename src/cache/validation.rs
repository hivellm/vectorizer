...
impl CacheValidator {
    ...
    pub async fn validate_metadata(&self, metadata: &CacheMetadata) -> ValidationResult {
        ...
        self.validate_metadata_structure(metadata, &mut result).await;
        
        // Validate collections
        for (collection_name, collection_info) in &metadata.collections {
            self.validate_collection(collection_name, collection_info, &mut result).await;
        }
        
        // Calculate statistics
        result.stats.total_collections = metadata.collections.len();
        result.stats.total_files = metadata.collections.values()
    ...
    /// Validate metadata structure
    async fn validate_metadata_structure(&self, metadata: &CacheMetadata, result: &mut ValidationResult) {
        // Check version
    ...
    async fn validate_collection(
        ...
        if collection_info.name != collection_name {
            result.errors.push(ValidationError {
                ...
                message: format!("Collection name mismatch: {} != {}", collection_name, collection_info.name),
                file_path: None,
        ...
        for (file_path, file_info) in &collection_info.file_hashes {
            self.validate_file(file_path, file_info, collection_name, result).await;
        }
            ...
            if let Ok(metadata) = std::fs::metadata(file_path) {
                if metadata.len() != file_info.size {
                    result.errors.push(ValidationError {
                        ...
                        message: format!("File size mismatch: expected {}, found {}", file_info.size, metadata.len()),
                        file_path: Some(file_path_str.clone()),
                        collection_name: Some(collection_name.to_string()),
                    });
                }
            ...
            if let Ok(calculated_hash) = self.calculate_file_hash(file_path).await {
                if calculated_hash != file_info.content_hash {
                    result.errors.push(ValidationError {
                        ...
                        message: format!("Content hash mismatch: expected {}, found {}", file_info.content_hash, calculated_hash),
                        file_path: Some(file_path_str.clone()),
                        collection_name: Some(collection_name.to_string()),
                    });
                }
    ...
    fn check_duplicate_hashes(
        ...
        for (file_path, file_info) in &collection_info.file_hashes {
            let file_path_str = file_path.to_string_lossy().to_string();
            hash_counts.entry(file_info.content_hash.clone())
    ...
    pub fn summary(&self) -> String {
        match self.status {
            ...
            ValidationStatus::ValidWithWarnings => {
                format!("Cache validation passed with {} warnings", self.warning_count())
            }
