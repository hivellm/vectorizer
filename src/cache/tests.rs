...
async fn test_cache_metadata_operations() {
    ...
    manager.update_collection_info(collection_info.clone()).await.unwrap();
    
    assert!(manager.has_collection("test_collection").await);
    
    let retrieved_info = manager.get_collection_info("test_collection").await.unwrap();
...
async fn test_cache_cleanup() {
    ...
    manager.update_collection_info(collection_info).await.unwrap();
...
async fn test_incremental_processor() {
    ...
    let processor = IncrementalProcessor::new(cache_manager, incremental_config).await.unwrap();
