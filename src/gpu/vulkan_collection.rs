...
impl VulkanCollection {
    ...
    pub async fn new(
        ...
        info!(
            "🔥 Creating Vulkan GPU-accelerated collection '{}' with complete GPU storage",
            name
        );
        ...
        info!(
            "✅ Vulkan GPU initialized: {} for collection '{}'",
            gpu_info.name, name
        );
    ...
    pub async fn add_vector(&self, vector: Vector) -> Result<()> {
        debug!(
            "Adding vector '{}' to Vulkan collection '{}' with GPU storage",
            vector.id, self.name
        );
        ...
        debug!(
            "Added HNSW node at index {} for vector '{}'",
            node_index, vector.id
        );
        ...
        *self.updated_at.write() = chrono::Utc::now();
        ...
        debug!(
            "After adding vector '{}': hnsw_nodes={}, vector_count={}",
            vector.id, hnsw_stats.node_count, vector_stats.vector_count
        );

        debug!(
            "Successfully added vector '{}' to Vulkan GPU storage",
            vector.id
        );
        Ok(())
    }
    ...
    pub async fn search(&self, query: &[f32], k: usize) -> Result<Vec<SearchResult>> {
        debug!(
            "Searching Vulkan collection '{}' with complete GPU acceleration",
            self.name
        );
        ...
        debug!(
            "Vulkan search stats: hnsw_nodes={}, vector_count={}",
        ...
        debug!(
            "Vulkan GPU search completed, found {} results",
            results.len()
        );
        Ok(results)
    }
    ...
    pub fn remove_vector(&self, id: &str) -> Result<()> {
        debug!(
            "Removing vector '{}' from Vulkan GPU collection '{}'",
            id, self.name
        );
        ...
        *self.updated_at.write() = chrono::Utc::now();
        debug!(
            "Successfully removed vector '{}' from Vulkan GPU storage",
            id
        );
        Ok(())
    }
...
mod tests {
    ...
    async fn test_vulkan_gpu_memory_stats() {
        ...
        if let Ok(collection) = VulkanCollection::new("test".to_string(), config, gpu_config).await
        {
            ...
            println!(
                "Vulkan GPU memory stats: {}MB used / {}MB limit",
