...
impl GpuHnswSearch {
    ...
    pub async fn batch_distance_gpu(
        ...
        let distances = match self.metric {
            DistanceMetric::Cosine => {
                self.gpu_context
                    .cosine_similarity(query, candidates)
                    .await?
            }
            DistanceMetric::Euclidean => {
                self.gpu_context
                    .euclidean_distance(query, candidates)
                    .await?
            }
            DistanceMetric::DotProduct => self.gpu_context.dot_product(query, candidates).await?,
        };
        ...
        // Update statistics
        {
            ...
            stats.gpu_batches += 1;
            stats.avg_batch_size = (stats.avg_batch_size * (stats.gpu_batches - 1) as f64
                + candidates.len() as f64)
                / stats.gpu_batches as f64;
        }
    ...
    pub async fn evaluate_candidates_gpu(
        ...
        for (batch_ids, batch_vectors) in ids
...
mod tests {
    ...
    async fn test_gpu_hnsw_search_creation() {
        ...
        let search = GpuHnswSearch::new(gpu_ctx, hnsw_config, DistanceMetric::Cosine);
    ...
    async fn test_batch_distance_gpu() {
        ...
        let candidates: Vec<Vec<f32>> = (0..10).map(|_| generate_random_vector(128)).collect();
    ...
    async fn test_evaluate_candidates() {
        ...
        let candidates: Vec<(usize, Vec<f32>)> =
            (0..100).map(|i| (i, generate_random_vector(128))).collect();
