...
impl DirectX12Collection {
    ...
    pub async fn new(
        ...
        let hnsw_storage_config = GpuHnswStorageConfig {
            ...
            gpu_memory_limit: 4 * 1024 * 1024 * 1024, // 4GB for DirectX 12
        };
    ...
    pub fn get_gpu_memory_stats(&self) -> DirectX12GpuMemoryStats {
        let hnsw_stats = self.hnsw_storage.get_memory_stats();
        let vector_stats = self.vector_storage.get_storage_stats();

        DirectX12GpuMemoryStats {
    ...
    pub fn gpu_info(&self) -> String {
        ...
        format!("{} (DirectX 12 GPU Full Storage)", info.name)
    }
...
/// DirectX 12 GPU Memory Statistics
#[derive(Debug, Clone)]
pub struct DirectX12GpuMemoryStats {
...
mod tests {
    ...
    async fn test_dx12_collection_creation() {
        ...
        match result {
            Ok(collection) => {
                ...
                println!("DirectX 12 GPU full storage collection created successfully");
            }
    ...
    #[tokio::test]
    async fn test_dx12_gpu_memory_stats() {
        let config = CollectionConfig {
            dimension: 128,
            metric: DistanceMetric::Cosine,
            hnsw_config: HnswConfig::default(),
