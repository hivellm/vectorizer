...
impl ScalarQuantization {
    ...
    pub fn new(bits: u8) -> QuantizationResult<Self> {
        if !matches!(bits, 1 | 2 | 4 | 8) {
            return Err(QuantizationError::InvalidParameters(
                format!("Invalid bit depth: {}. Must be 1, 2, 4, or 8", bits)
            ));
        }
    ...
    pub fn quantize_vector(&self, vector: &[f32]) -> QuantizationResult<Vec<u8>> {
        match self.bits {
            ...
            _ => Err(QuantizationError::InvalidParameters(
                format!("Unsupported bit depth: {}", self.bits)
            )),
        }
    }
    ...
    fn dequantize_1bit(&self, quantized: &[u8]) -> QuantizationResult<Vec<f32>> {
        ...
        for &packed in quantized {
            for i in 0..8 {
                let bit = (packed >> i) & 1;
                let value = if bit == 1 { self.max_value } else { self.min_value };
                dequantized.push(value);
            }
        }
    ...
    pub fn calculate_quantization_error(&self, original: &[f32], quantized: &[u8]) -> QuantizationResult<f32> {
...
impl QuantizationMethod for ScalarQuantization {
    ...
    fn dequantize(&self, quantized: &QuantizedVectors) -> QuantizationResult<Vec<Vec<f32>>> {
        ...
        let bytes_per_vector = match self.bits {
            ...
            _ => return Err(QuantizationError::InvalidParameters(
                format!("Unsupported bit depth: {}", self.bits)
            )),
        };
    ...
    fn validate_parameters(&self) -> QuantizationResult<()> {
        if !matches!(self.bits, 1 | 2 | 4 | 8) {
            return Err(QuantizationError::InvalidParameters(
                format!("Invalid bit depth: {}", self.bits)
            ));
        }
    ...
    fn deserialize_params(&mut self, params: QuantizationParams) -> QuantizationResult<()> {
        if let QuantizationParams::Scalar { bits, min_value, max_value, scale } = params {
            self.bits = bits;
...
impl QuantizedSearch for ScalarQuantization {
    ...
    fn quantized_similarity(&self, quantized_a: &[u8], quantized_b: &[u8]) -> QuantizationResult<f32> {
        ...
        // Calculate cosine similarity
        let dot_product: f32 = dequantized_a.iter()
...
mod tests {
    ...
    fn test_scalar_quantization_8bit() {
        ...
        for (orig, deq) in vectors.iter().zip(dequantized.iter()) {
            for (o, d) in orig.iter().zip(deq.iter()) {
                assert!((o - d).abs() < 0.1, "Quantization error too large: {} vs {}", o, d);
            }
        }
    }
    ...
    fn test_memory_usage_calculation() {
        ...
        assert_eq!(sq4.memory_usage(vector_count, dimension), 1000 * (512 + 1) / 2);
        assert_eq!(sq2.memory_usage(vector_count, dimension), 1000 * (512 + 3) / 4);
        assert_eq!(sq1.memory_usage(vector_count, dimension), 1000 * (512 + 7) / 8);
    }
    ...
    fn test_similarity_calculation() {
        ...
        assert!(similarity > 0.9, "Similarity should be high for identical vectors");
    }
