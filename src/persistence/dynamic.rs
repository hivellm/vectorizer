...
impl DynamicCollectionPersistence {
    ...
    pub async fn new(
        ...
        let wal = Arc::new(WriteAheadLog::new(wal_path, config.wal_config.clone()).await?);
        ...
        info!(
            "Dynamic collection persistence initialized at {}",
            persistence.base_path.display()
        );
        Ok(persistence)
    }
    ...
    pub async fn create_collection(
        ...
        info!(
            "Dynamic collection '{}' created with ID '{}'",
            name, metadata.id
        );
        Ok(metadata)
    }
    ...
    async fn save_metadata(
        &self,
        metadata: &EnhancedCollectionMetadata,
    ) -> Result<(), PersistenceError> {
        let metadata_path = self.metadata_path(&metadata.id);
        let json =
            serde_json::to_string_pretty(metadata).map_err(PersistenceError::SerializationError)?;
    ...
    async fn load_metadata(
        ...
        if !metadata_path.exists() {
            return Err(PersistenceError::CollectionNotFound(
        ...
        let content = fs::read_to_string(&metadata_path)
            .await
            .map_err(PersistenceError::IoError)?;
        let metadata: EnhancedCollectionMetadata =
            serde_json::from_str(&content).map_err(PersistenceError::SerializationError)?;
        ...
        if let Ok(mut entries) = entries {
            while let Ok(Some(entry)) = entries.next_entry().await {
                if let Ok(metadata) = self
                    .load_metadata(&entry.file_name().to_string_lossy())
                    .await
                {
    ...
    pub async fn list_collections(
        &self,
    ) -> Result<Vec<EnhancedCollectionMetadata>, PersistenceError> {
    ...
    pub async fn begin_transaction(&self, collection_id: &str) -> Result<u64, PersistenceError> {
        ...
        debug!(
            "Transaction {} started for collection {}",
    ...
    pub async fn delete_collection(&self, collection_name: &str) -> Result<(), PersistenceError> {
        ...
        if metadata.is_workspace() {
            return Err(PersistenceError::CannotDeleteWorkspace(
    ...
    async fn recover_collection(&self, collection_id: &str) -> Result<(), PersistenceError> {
        ...
        let new_entries: Vec<_> = wal_entries
            ...
            .filter(|entry| {
                entry
                    .transaction_id
                    .map_or(false, |id| id > last_transaction_id)
            })
            .collect();
    ...
    async fn verify_collection_integrity(
        &self,
        metadata: &EnhancedCollectionMetadata,
    ) -> Result<(), PersistenceError> {
        ...
        if let Some(stored_checksum) = &metadata.data_checksum {
            if calculated_checksum != *stored_checksum {
                warn!(
                    "Checksum mismatch for collection '{}': calculated={}, stored={}",
