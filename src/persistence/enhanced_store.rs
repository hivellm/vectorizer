use crate::persistence::{
    types::{EnhancedCollectionMetadata, CollectionType, CollectionSource},
    dynamic::{DynamicCollectionPersistence, PersistenceConfig, PersistenceError},
};
...
impl EnhancedVectorStore {
    ...
    async fn initialize_metadata_cache(&self) -> Result<(), EnhancedStoreError> {
        ...
        info!("Metadata cache initialized with {} collections", cache.len());
        Ok(())
    }

    /// Create workspace collection metadata
    async fn create_workspace_metadata(&self, collection_name: &str) -> Result<EnhancedCollectionMetadata, EnhancedStoreError> {
    ...
    pub async fn get_collection_metadata(&self, collection_name: &str) -> Result<EnhancedCollectionMetadata, EnhancedStoreError> {
        let cache = self.metadata_cache.read().await;
        
        if let Some(metadata) = cache.get(collection_name) {
            Ok(metadata.clone())
        } else {
            Err(EnhancedStoreError::CollectionNotFound(collection_name.to_string()))
        }
    }
    ...
    pub async fn create_collection(
        ...
        let metadata = self.dynamic_persistence.create_collection(name.clone(), config, created_by).await?;
    ...
    pub async fn delete_collection(&self, collection_name: &str) -> Result<(), EnhancedStoreError> {
        ...
        if metadata.is_workspace() {
            return Err(EnhancedStoreError::CannotDeleteWorkspace(collection_name.to_string()));
        }

        // Delete dynamic collection
        self.dynamic_persistence.delete_collection(collection_name).await?;
    ...
    pub async fn insert_vectors(
        ...
        if metadata.is_read_only {
            return Err(EnhancedStoreError::ReadOnlyCollection(collection_name.to_string()));
        }

        // For dynamic collections, use persistence manager
        if metadata.is_dynamic() {
            // Begin transaction
            let transaction_id = self.dynamic_persistence.begin_transaction(&metadata.id).await?;
            ...
            for vector in vectors {
                ...
                self.dynamic_persistence.add_to_transaction(transaction_id, operation).await?;
            }
            
            // Commit transaction
            self.dynamic_persistence.commit_transaction(transaction_id).await?;
    ...
    pub async fn get_collection_stats(&self, collection_name: &str) -> Result<CollectionStats, EnhancedStoreError> {
        let metadata = self.get_collection_metadata(collection_name).await?;
...
mod tests {
    ...
    async fn create_test_enhanced_store() -> EnhancedVectorStore {
        ...
        EnhancedVectorStore::new(workspace_store, config).await.unwrap()
    }
    ...
    async fn test_list_collections() {
        ...
        store.create_collection("collection1".to_string(), config.clone(), None).await.unwrap();
        store.create_collection("collection2".to_string(), config, None).await.unwrap();
    ...
    async fn test_delete_dynamic_collection() {
        ...
        // Create collection
        store.create_collection("test-collection".to_string(), config, None).await.unwrap();
        assert!(store.collection_exists("test-collection").await);
