...
static REGISTRY: Lazy<Arc<RwLock<Registry>>> = Lazy::new(|| {
...
mod tests {
    ...
    fn test_registry_access() {
        ...
        assert!(!metric_families.is_empty(), "Registry should contain metrics");
    }
    ...
    fn test_metrics_export() {
        ...
        // Record some metrics
        METRICS.search_requests_total.with_label_values(&["test_export", "basic", "success"]).inc();
        ...
        assert!(output.contains("vectorizer_search_requests_total"), "Should contain search requests metric");
        assert!(output.contains("vectorizer_vectors_total"), "Should contain vectors total metric");
    }
}
