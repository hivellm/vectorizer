...
use super::{CliConfig, ServerCommands, UserCommands, ApiKeyCommands, CollectionCommands, DbCommands, ConfigCommands};
use crate::auth::{AuthManager, Role, Permission};
...
pub async fn handle_server_command(command: ServerCommands, config: &CliConfig) -> Result<()> {
    match command {
        ServerCommands::Start { host, port, auth, data_dir } => {
            ...
            info!("Authentication: {}", if auth { "enabled" } else { "disabled" });
...
pub async fn handle_user_command(command: UserCommands, config: &CliConfig) -> Result<()> {
    ...
    match command {
        UserCommands::Create { username, roles, description } => {
...
pub async fn handle_api_key_command(command: ApiKeyCommands, config: &CliConfig) -> Result<()> {
    ...
    match command {
        ApiKeyCommands::Create { user_id, name, permissions, expires_in_hours } => {
            ...
            if let Some(expires_at) = expires_at {
                info!("Expires: {}", chrono::DateTime::from_timestamp(expires_at as i64, 0).unwrap());
            } else {
                info!("Expires: Never");
            }
            ...
            if let Some(user_id) = user_id {
                ...
                for key in keys {
                    if detailed {
                        ...
                        info!("  Created: {}", chrono::DateTime::from_timestamp(key.created_at as i64, 0).unwrap());
                        if let Some(last_used) = key.last_used {
                            info!("  Last Used: {}", chrono::DateTime::from_timestamp(last_used as i64, 0).unwrap());
                        }
                        info!("  Active: {}", key.active);
                        info!("");
                    } else {
                        info!("{} - {} ({})", key.id, key.name, if key.active { "active" } else { "inactive" });
                    }
                }
            } else {
        ...
        ApiKeyCommands::Test { api_key } => {
            ...
            match auth_manager.validate_api_key(&api_key).await {
                Ok(claims) => {
                    ...
                    info!("Expires: {}", chrono::DateTime::from_timestamp(claims.exp as i64, 0).unwrap());
                }
                Err(e) => {
...
pub async fn handle_collection_command(command: CollectionCommands, _config: &CliConfig) -> Result<()> {
    let store = VectorStore::new();
    
    match command {
        CollectionCommands::Create { name, dimension, metric } => {
        ...
        CollectionCommands::Stats { name } => {
            ...
            info!("    EF Construction: {}", metadata.config.hnsw_config.ef_construction);
            info!("    EF Search: {}", metadata.config.hnsw_config.ef_search);
            info!("  Compression: {}", if metadata.config.compression.enabled { "enabled" } else { "disabled" });
...
pub async fn handle_db_command(command: DbCommands, _config: &CliConfig) -> Result<()> {
    match command {
        DbCommands::Backup { output, collections } => {
        ...
        DbCommands::Optimize { rebuild_indexes, cleanup } => {
...
pub async fn handle_config_command(command: ConfigCommands, config: &CliConfig) -> Result<()> {
    match command {
        ConfigCommands::Show { show_secrets } => {
            ...
            info!("  Rate Limit (per minute): {}", config.auth.rate_limit_per_minute);
            info!("  Rate Limit (per hour): {}", config.auth.rate_limit_per_hour);
            ...
            info!("  Compression Enabled: {}", config.database.compression_enabled);
            info!("  Compression Threshold: {} bytes", config.database.compression_threshold);
        ...
        ConfigCommands::Validate { file } => {
            ...
            if !file.exists() {
                return Err(crate::error::VectorizerError::NotFound(format!("Configuration file not found: {:?}", file)));
            }
...
pub async fn handle_status_command(detailed: bool, config: &CliConfig) -> Result<()> {
    ...
    info!("  Port: {}", config.server.port);
    info!("  Data Directory: {:?}", config.server.data_dir);
    info!("  Authentication: {}", if config.server.auth_enabled { "enabled" } else { "disabled" });
    
    if detailed {
        info!("Detailed Status:");
        info!("  Version: {}", env!("CARGO_PKG_VERSION"));
        info!("  Rust Version: {}", std::env::var("RUSTC_VERSION").unwrap_or_else(|_| "unknown".to_string()));
        info!("  Build Time: {}", std::env::var("BUILD_TIME").unwrap_or_else(|_| "unknown".to_string()));
        ...
        info!("  Threshold: {} bytes", config.database.compression_threshold);
    }
