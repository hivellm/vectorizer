...
impl MetricsCollector {
    ...
    pub async fn get_metrics(&self) -> FileWatcherMetrics {
        ...
        let total_api_response_time = self.metrics.total_api_response_time_ms.load(Ordering::Relaxed);
        
        let uptime = self.metrics.start_time.elapsed().as_secs();
        let last_activity = self.last_activity.read().await.map(|t| {
        ...
        let avg_discovery_ms = if self.metrics.files_discovered.load(Ordering::Relaxed) > 0 {
            total_discovery_time as f64 / self.metrics.files_discovered.load(Ordering::Relaxed) as f64
        } else {
            0.0
        };
        ...
        FileWatcherMetrics {
            ...
            files: FileMetrics {
                ...
                files_processed_success: self.metrics.files_processed_success.load(Ordering::Relaxed),
            ...
            network: NetworkMetrics {
                total_api_requests,
                successful_api_requests: self.metrics.successful_api_requests.load(Ordering::Relaxed),
    ...
    pub async fn reset(&self) {
        ...
        self.metrics.total_files_processed.store(0, Ordering::Relaxed);
        self.metrics.files_processed_success.store(0, Ordering::Relaxed);
        self.metrics.files_processed_error.store(0, Ordering::Relaxed);
        ...
        self.metrics.files_indexed_realtime.store(0, Ordering::Relaxed);
        self.metrics.total_processing_time_ms.store(0, Ordering::Relaxed);
        self.metrics.total_discovery_time_ms.store(0, Ordering::Relaxed);
        ...
        self.metrics.successful_api_requests.store(0, Ordering::Relaxed);
        self.metrics.failed_api_requests.store(0, Ordering::Relaxed);
        self.metrics.total_api_response_time_ms.store(0, Ordering::Relaxed);
        self.metrics.peak_api_response_ms.store(0, Ordering::Relaxed);
    ...
    pub async fn get_summary(&self) -> String {
        ...
        format!("Files processed: {}, Success: {}, Errors: {}, Uptime: {}s", 
    ...
    pub async fn record_file_processing_complete(&self, success: bool, processing_time_ms: f64) {
        self.metrics.total_files_processed.fetch_add(1, Ordering::Relaxed);
        self.metrics.total_processing_time_ms.fetch_add(processing_time_ms as u64, Ordering::Relaxed);
        
        if success {
            self.metrics.files_processed_success.fetch_add(1, Ordering::Relaxed);
        } else {
            self.metrics.files_processed_error.fetch_add(1, Ordering::Relaxed);
        ...
        if processing_time_ms as u64 > current_peak {
            self.metrics.peak_processing_ms.store(processing_time_ms as u64, Ordering::Relaxed);
        }
    ...
    pub async fn record_discovery(&self, files_found: u64, discovery_time_ms: f64) {
        self.metrics.files_discovered.fetch_add(files_found, Ordering::Relaxed);
        self.metrics.total_discovery_time_ms.fetch_add(discovery_time_ms as u64, Ordering::Relaxed);
        *self.last_activity.write().await = Some(Instant::now());
    }

    /// Record sync operation
    pub async fn record_sync(&self, orphaned_removed: u64, unindexed_found: u64, sync_time_ms: f64) {
        self.metrics.files_removed.fetch_add(orphaned_removed, Ordering::Relaxed);
        self.metrics.files_indexed_realtime.fetch_add(unindexed_found, Ordering::Relaxed);
        self.metrics.total_sync_time_ms.fetch_add(sync_time_ms as u64, Ordering::Relaxed);
        *self.last_activity.write().await = Some(Instant::now());
    }
    ...
    pub async fn update_collection_metrics(&self, collection_name: &str, total_vectors: u64, size_bytes: u64) {
        ...
        let collection = collections.entry(collection_name.to_string()).or_insert_with(|| CollectionMetrics {
            total_vectors: 0,
            vectors_added: 0,
            vectors_removed: 0,
            vectors_updated: 0,
    ...
    pub fn record_file_in_progress(&self) {
        self.metrics.files_in_progress.fetch_add(1, Ordering::Relaxed);
    }

    /// Record file processing finished
    pub fn record_file_processing_finished(&self) {
        self.metrics.files_in_progress.fetch_sub(1, Ordering::Relaxed);
    }

    /// Record API request
    pub fn record_api_request(&self, success: bool, response_time_ms: f64) {
        self.metrics.total_api_requests.fetch_add(1, Ordering::Relaxed);
        self.metrics.total_api_response_time_ms.fetch_add(response_time_ms as u64, Ordering::Relaxed);
        
        if success {
            self.metrics.successful_api_requests.fetch_add(1, Ordering::Relaxed);
        } else {
            self.metrics.failed_api_requests.fetch_add(1, Ordering::Relaxed);
        }
        ...
        if response_time_ms as u64 > current_peak {
            self.metrics.peak_api_response_ms.store(response_time_ms as u64, Ordering::Relaxed);
        }
    }
    ...
    pub fn record_network_io(&self, bytes: u64) {
        self.metrics.network_io_bytes.fetch_add(bytes, Ordering::Relaxed);
    }
