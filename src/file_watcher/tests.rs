...
mod tests {
    ...
    async fn test_enhanced_file_watcher_creation() {
        ...
        let enhanced_watcher =
            EnhancedFileWatcher::new(config.clone(), debouncer, hash_validator).unwrap();
    ...
    async fn test_enhanced_file_watcher_success() {
        ...
        let hash2 = hash_validator
            .calculate_content_hash("fn main() { println!(\"Different!\"); }")
            .await;
        ...
        assert!(
            file_index.contains_file(&test_file),
        ...
        assert_eq!(
            ...
            "Should have 0 files after removal"
        );
        ...
        vector_store
            .create_collection("test-collection", collection_config)
            .unwrap();
        ...
        vector_store
            .insert("test-collection", vec![vector])
            .unwrap();
        println!("✅ Vector inserted");

        // Check collection metadata
        let metadata = vector_store
            .get_collection_metadata("test-collection")
            .unwrap();
        assert_eq!(
            metadata.vector_count, 1,
            "Should have 1 vector in collection"
        );
        ...
        let enhanced_watcher = EnhancedFileWatcher::new(config, debouncer, hash_validator).unwrap();
    ...
    async fn test_dynamic_collection_workflow() {
        ...
        // Create enhanced watcher
        let mut enhanced_watcher =
            EnhancedFileWatcher::new(config.clone(), debouncer, hash_validator.clone()).unwrap();
        ...
        vector_store
            .insert("dynamic-test-collection ", vec![vector])
            .unwrap();
        ...
        let collection_metadata = vector_store
            .get_collection_metadata("dynamic-test-collection ")
            .unwrap();
        assert_eq!(
            collection_metadata.vector_count, 1,
            "Should have 1 vector in collection "
        );
        println!(
            "Collection has {} vectors ",
            collection_metadata.vector_count
        );
        ...
        let new_hash = hash_validator
            .calculate_content_hash(&modified_content)
            .await;
        assert_ne!(
            content_hash, new_hash,
            "File modification should change hash "
        );
        ...
        vector_store
            .insert("dynamic-test-collection ", vec![new_vector])
            .unwrap();
        ...
        let final_collection_metadata = vector_store
            .get_collection_metadata("dynamic-test-collection ")
            .unwrap();
        assert_eq!(
            final_collection_metadata.vector_count, 2,
            "Should have 2 vectors in collection "
        );
        println!(
            "Final collection has {} vectors ",
            final_collection_metadata.vector_count
        );
