...
impl FileDiscovery {
    ...
    pub async fn discover_existing_files(&self) -> Result<DiscoveryResult, Box<dyn std::error::Error + Send + Sync>> {
        ...
        match discovery_result {
            Ok(result) => result,
            Err(_) => {
                warn!("⚠️ File discovery timed out after {} seconds", DISCOVERY_TIMEOUT_SECONDS);
                Ok(DiscoveryResult {
    ...
    async fn perform_discovery(&self) -> Result<DiscoveryResult, Box<dyn std::error::Error + Send + Sync>> {
        let start_time = std::time::Instant::now();
        ...
        info!("📁 Scanning {} directories for existing files", watch_paths.len());
        ...
        info!("✅ File discovery completed in {}ms", stats.processing_time_ms);
        info!("📊 Discovery stats: {} scanned, {} indexed, {} skipped, {} errors", 
    ...
    async fn scan_directory(&self, path: &Path) -> Result<DiscoveryResult, Box<dyn std::error::Error + Send + Sync>> {
        let mut stats = DiscoveryStats {
            total_files_scanned: 0,
            files_indexed: 0,
            files_skipped: 0,
        ...
        if files.len() > MAX_FILES_PER_DISCOVERY {
            warn!("⚠️ Found {} files, limiting to {} for discovery to avoid system overload", files.len(), MAX_FILES_PER_DISCOVERY);
        ...
        for (index, file_path) in files.iter().enumerate() {
            info!("📄 Processing file {}/{}: {:?}", index + 1, files.len(), file_path);
            ...
            match Self::process_single_file(file_path, &self.config, &self.vector_operations).await {
                Ok(ProcessResult::Indexed) => {
                    ...
                    info!("✅ Indexed file {}/{}: {:?}", index + 1, files.len(), file_path);
                }
                Ok(ProcessResult::Skipped(reason)) => {
                    stats.files_skipped += 1;
                    skipped_files.push(file_path.clone());
                    info!("⏭️ Skipped file {}/{}: {:?} - {}", index + 1, files.len(), file_path, reason);
                }
                Err(e) => {
                    stats.files_errors += 1;
                    error_files.push((file_path.clone(), e.to_string()));
                    warn!("❌ Error processing file {}/{}: {:?} - {}", index + 1, files.len(), file_path, e);
                }
            }
        }
    ...
    async fn collect_files_recursive(&self, path: &Path) -> Result<Vec<PathBuf>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn process_single_file(
        ...
        if !config.should_process_file(file_path) {
            return Ok(ProcessResult::Skipped("Does not match include patterns".to_string()));
        }

        // Check file size
        if let Ok(metadata) = std::fs::metadata(file_path) {
            if metadata.len() > config.max_file_size {
                return Ok(ProcessResult::Skipped(format!("File too large: {} bytes", metadata.len())));
            }
        }
    ...
    async fn get_watch_paths(&self) -> Result<Vec<PathBuf>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn discover_workspace_paths(&self) -> Result<Vec<PathBuf>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if !workspace_file.exists() {
            ...
            return Ok(vec![std::env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."))]);
        }
        ...
        info!("🔍 Discovered {} watch paths from workspace config", watch_paths.len());
        Ok(watch_paths)
    }

    /// Sync with existing collections to remove orphaned files
    pub async fn sync_with_existing_collections(&self) -> Result<SyncResult, Box<dyn std::error::Error + Send + Sync>> {
        ...
        for collection_name in collections {
            ...
            if !orphaned_files.is_empty() {
                info!("🗑️ Removing {} orphaned files from collection '{}'", orphaned_files.len(), collection_name);
                ...
                info!("✅ No orphaned files found in collection '{}'", collection_name);
            }
        }
        ...
        info!("📊 Sync results: {} collections checked, {} files verified, {} orphaned files removed", 
    ...
    pub async fn detect_unindexed_files(&self) -> Result<Vec<PathBuf>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        for watch_path in watch_paths {
            ...
            info!("📄 Found {} files to check in {:?}", files.len(), watch_path);
    ...
    pub async fn comprehensive_sync(&self) -> Result<(SyncResult, Vec<PathBuf>), Box<dyn std::error::Error + Send + Sync>> {
