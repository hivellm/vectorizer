...
impl Indexer {
    ...
    pub fn build_vocabulary(&mut self, documents: &[(PathBuf, String)]) -> Result<()> {
        ...
        info!("Building vocabulary from {} documents for provider: {}", texts.len(), self.config.embedding_type);
        ...
        if let Some(emb) = self.embedding_manager.get_provider_mut(provider) {
            ...
            if let Some(bm25) = emb.as_any_mut().downcast_mut::<crate::embedding::Bm25Embedding>() {
                bm25.build_vocabulary(&texts);
                info!("✅ Built BM25 vocabulary with {} documents, vocab size: {}", texts.len(), bm25.vocabulary_size());
            } else if let Some(tfidf) = emb.as_any_mut().downcast_mut::<crate::embedding::TfIdfEmbedding>() {
                // TF-IDF expects &[&str]
                let text_refs: Vec<&str> = texts.iter().map(|s| s.as_str()).collect();
                tfidf.build_vocabulary(&text_refs);
                info!("✅ Built TF-IDF vocabulary with {} documents", texts.len());
            } else if let Some(bow) = emb.as_any_mut().downcast_mut::<crate::embedding::BagOfWordsEmbedding>() {
                // BagOfWords expects &[&str]
                let text_refs: Vec<&str> = texts.iter().map(|s| s.as_str()).collect();
                bow.build_vocabulary(&text_refs);
                info!("✅ Built Bag-of-Words vocabulary with {} documents", texts.len());
            } else if let Some(charngram) = emb.as_any_mut().downcast_mut::<crate::embedding::CharNGramEmbedding>() {
                // CharNGram expects &[&str]
                let text_refs: Vec<&str> = texts.iter().map(|s| s.as_str()).collect();
                charngram.build_vocabulary(&text_refs);
                info!("✅ Built CharNGram vocabulary with {} documents", texts.len());
            } else {
                info!("Provider '{}' does not require vocabulary building", provider);
            }
        } else {
