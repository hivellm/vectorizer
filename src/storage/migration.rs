...
impl StorageMigrator {
    ...
    fn create_backup(&self) -> Result<PathBuf> {
        ...
        fs::create_dir_all(&backup_dir).map_err(|e| VectorizerError::Io(e))?;
    ...
    fn copy_dir_recursive(&self, src: &Path, dst: &Path) -> Result<()> {
        fs::create_dir_all(dst).map_err(|e| VectorizerError::Io(e))?;
    ...
    pub fn rollback(&self, backup_path: &Path) -> Result<()> {
        ...
        if vecdb_path.exists() {
            fs::remove_file(&vecdb_path).map_err(|e| VectorizerError::Io(e))?;
        }

        if vecidx_path.exists() {
            fs::remove_file(&vecidx_path).map_err(|e| VectorizerError::Io(e))?;
        }
