...
use crate::storage::{StorageIndex, StorageReader, StorageWriter};
...
impl StorageCompactor {
    ...
    pub fn compact_from_memory(&self, store: &crate::db::VectorStore) -> Result<StorageIndex> {
        ...
        if vecdb_path.exists() {
            let backup_path = self
                .data_dir
                .join(format!("{}.backup", crate::storage::VECDB_FILE));
            match std::fs::copy(&vecdb_path, &backup_path) {
                ...
                Err(e) => {
                    warn!(
                        "⚠️  Failed to create backup: {} - proceeding with caution",
                        e
                    );
                }
            }
        ...
        let total_vectors: usize = persisted_collections.iter().map(|c| c.vectors.len()).sum();

        if total_vectors == 0 {
            error!(
                "❌ CRITICAL: All {} collections have ZERO vectors!",
            ...
            if vecdb_path.exists() {
                if let Ok(metadata) = std::fs::metadata(&vecdb_path) {
                    ...
                    if size_mb > 0 {
                        error!(
                            "   REFUSING to overwrite {} MB of existing data with empty collections!",
                            size_mb
                        );
                    }
                }
            ...
            return Err(VectorizerError::Storage(format!(
                "Refusing to overwrite vectorizer.vecdb - {} collections but 0 total vectors",
                persisted_collections.len()
            )));
        }

        info!(
            "✅ Safety check passed: {} total vectors across {} collections",
            total_vectors,
            persisted_collections.len()
        );
    ...
    pub fn maybe_compact(&mut self) -> Result<Option<StorageIndex>> {
        if self.should_compact() {
            info!(
                "🔄 Triggering incremental compaction ({} pending operations)",
    ...
    pub fn verify_integrity(&self) -> Result<bool> {
        ...
        for collection_name in &collections {
            let collection = reader.get_collection(collection_name)?.ok_or_else(|| {
                VectorizerError::Storage("Collection not found in index".to_string())
            })?;
    ...
    pub fn compact_all_with_cleanup(&self, remove_source_files: bool) -> Result<StorageIndex> {
        info!(
            "🗜️  Starting compaction (remove_source_files: {})",
            remove_source_files
        );
        ...
        if !self.verify_integrity()? {
            ...
            return Err(VectorizerError::Storage(
                "Integrity check failed after compaction".to_string(),
            ));
        }
