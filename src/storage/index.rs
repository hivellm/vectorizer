...
impl FileEntry {
    ...
    pub fn new(path: String, size: u64, compressed_size: u64, checksum: String, file_type: FileType) -> Self {
        Self {
            path,
            size,
            compressed_size,
...
pub fn detect_file_type(path: &str) -> FileType {
    ...
    if path_lower.ends_with(".bin") || path_lower.ends_with(".bin.gz") {
        ...
        FileType::Metadata
    } else if path_lower.ends_with(".json") || path_lower.ends_with(".yaml") || path_lower.ends_with(".yml") {
        FileType::Config
...
mod tests {
    ...
    fn test_detect_file_type() {
        ...
        assert_eq!(detect_file_type("collection_metadata.json"), FileType::Metadata);
