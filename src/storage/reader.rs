...
impl StorageReader {
    ...
    fn read_file_from_archive(&self, path: &str) -> Result<Vec<u8>> {
        let file = File::open(&self.vecdb_path).map_err(|e| VectorizerError::Io(e))?;
    ...
    pub fn read_collection_files(&self, collection_name: &str) -> Result<HashMap<String, Vec<u8>>> {
        let collection = self.get_collection(collection_name)?.ok_or_else(|| {
            VectorizerError::Storage(format!("Collection not found: {}", collection_name))
        })?;
    ...
    pub fn extract_all_collections(&self) -> Result<Vec<crate::persistence::PersistedCollection>> {
        ...
        info!(
            "✅ Extracted {} collections from archive (no temp files created)",
    ...
    pub fn read_collection_in_memory(
        &self,
        collection_name: &str,
    ) -> Result<Option<crate::persistence::PersistedCollection>> {
        ...
        let vector_data = files.get(&vector_store_path).ok_or_else(|| {
            VectorizerError::Storage(format!(
                "Vector store file not found for collection: {}",
                collection_name
            ))
        })?;

        // Files are saved as JSON, not bincode
        let json_str = std::str::from_utf8(vector_data).map_err(|e| {
            VectorizerError::Deserialization(format!("Invalid UTF-8 in vector store: {}", e))
        })?;
        ...
        let persisted_store: crate::persistence::PersistedVectorStore =
            serde_json::from_str(json_str).map_err(|e| {
                VectorizerError::Deserialization(format!("Failed to deserialize collection: {}", e))
            })?;

        // Extract the first collection (files are saved as PersistedVectorStore with one collection)
        let mut persisted = persisted_store
            .collections
            .into_iter()
            .next()
            .ok_or_else(|| {
        ...
        if persisted.name.is_empty() {
            debug!(
                "⚠️  Collection name missing in persisted data, inferring from filename: '{}'",
                collection_name
            );
        ...
        if persisted.config.is_none() {
            ...
            if let Some(metadata_data) = files.get(&metadata_path) {
                let metadata_str = std::str::from_utf8(metadata_data).map_err(|e| {
                    VectorizerError::Deserialization(format!("Invalid UTF-8 in metadata: {}", e))
                })?;
        ...
        debug!(
            "✅ Collection '{}' loaded with {} vectors",
            persisted.name,
