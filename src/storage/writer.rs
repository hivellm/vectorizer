...
impl StorageWriter {
    ...
    pub fn write_archive(&self, collections_dir: &Path) -> Result<StorageIndex> {
        ...
        let temp_vecidx =
            vecidx_path.with_extension(format!("vecidx{}", crate::storage::TEMP_SUFFIX));
        ...
        fs::rename(&temp_vecdb, &vecdb_path).map_err(|e| VectorizerError::Io(e))?;
        fs::rename(&temp_vecidx, &vecidx_path).map_err(|e| VectorizerError::Io(e))?;
    ...
    pub fn write_from_memory(
        ...
        info!(
            "🗜️  Writing {} collections from memory to {}",
            collections.len(),
            temp_vecdb.display()
        );
    ...
    fn create_archive(&self, archive_path: &Path, source_dir: &Path) -> Result<StorageIndex> {
        let file = File::create(archive_path).map_err(|e| VectorizerError::Io(e))?;
    ...
    fn create_archive_from_memory(
        ...
        for persisted_collection in collections {
            ...
            let dimension = persisted_collection
                .config
                .as_ref()
                .map(|c| c.dimension)
                .unwrap_or(512);
            ...
            let json_data = serde_json::to_vec(&wrapped_store).map_err(|e| {
                VectorizerError::Serialization(format!("Failed to serialize collection: {}", e))
            })?;

            let original_size = json_data.len() as u64;

            // Write to ZIP
            zip.start_file(&vector_store_name, options).map_err(|e| {
                VectorizerError::Storage(format!("Failed to start ZIP file: {}", e))
            })?;
            ...
            if let Some(config) = config_clone {
                ...
                let metadata_json = serde_json::to_vec_pretty(&metadata).map_err(|e| {
                    VectorizerError::Serialization(format!("Failed to serialize metadata: {}", e))
                })?;

                zip.start_file(&metadata_name, options).map_err(|e| {
                    VectorizerError::Storage(format!("Failed to start metadata file: {}", e))
                })?;
            ...
            if checksums_path.exists() {
                ...
                match fs::read(&checksums_path) {
                    ...
                    Err(e) => {
                        warn!("   ⚠️  Failed to read checksums file: {}", e);
                    }
                }
            }
    ...
    fn discover_collections(
        ...
        info!(
            "🔍 Discovery complete: {} total files, {} .bin files, {} collections",
            total_files,
            bin_files,
            collections.len()
        );
        ...
        for entry in WalkDir::new(collection_dir) {
            ...
            if path.is_file() {
                let relative_path = path
                    .strip_prefix(collection_dir.parent().unwrap_or(collection_dir))
                    .map_err(|e| VectorizerError::Storage(e.to_string()))?;
    ...
    fn add_file_to_archive(
        ...
        let mut file = File::open(file_path).map_err(|e| VectorizerError::Io(e))?;
