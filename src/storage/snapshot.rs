...
impl SnapshotManager {
    ...
    pub fn new(data_dir: impl AsRef<Path>, snapshots_path: impl AsRef<Path>, max_snapshots: usize, retention_days: u64) -> Self {
    ...
    pub fn create_snapshot(&self) -> Result<SnapshotInfo> {
        ...
        if vecdb_src.exists() {
            fs::copy(&vecdb_src, &vecdb_dst)
                .map_err(|e| VectorizerError::Io(e))?;
        } else {
            return Err(VectorizerError::Storage("No .vecdb file to snapshot".to_string()));
        }
        ...
        if vecidx_src.exists() {
            fs::copy(&vecidx_src, &vecidx_dst)
                .map_err(|e| VectorizerError::Io(e))?;
        }
        
        // Get file size
        let vecdb_size = fs::metadata(&vecdb_dst)
            .map(|m| m.len())
            .unwrap_or(0);
    ...
    pub fn restore_snapshot(&self, id: &str) -> Result<()> {
        ...
        if vecdb_src.exists() {
            ...
            return Err(VectorizerError::Storage("Snapshot .vecdb file not found".to_string()));
        }
    ...
    fn load_snapshot_metadata(&self, snapshot_dir: &Path) -> Result<SnapshotInfo> {
        ...
        if !metadata_path.exists() {
            ...
            let size_bytes = fs::metadata(&vecdb_path)
                .map(|m| m.len())
                .unwrap_or(0);
