...
use std::env;
use vectorizer::embedding::{EmbeddingManager, TfIdfEmbedding, Bm25Embedding, BagOfWordsEmbedding, CharNGramEmbedding};
...
fn main() {
    ...
    manager.register_provider("bagofwords".to_string(), Box::new(BagOfWordsEmbedding::new(512)));
    manager.register_provider("charngram".to_string(), Box::new(CharNGramEmbedding::new(512, 3)));
    ...
    if let Some(proj) = &project {
        ...
        if let Some(p) = manager.get_provider_mut("bm25") {
            if let Some(bm25) = p.as_any_mut().downcast_mut::<Bm25Embedding>() {
                ...
                if path.exists() { let _ = bm25.load_vocabulary_json(&path); }
            }
        }
        if let Some(p) = manager.get_provider_mut("tfidf") {
            if let Some(tfidf) = p.as_any_mut().downcast_mut::<TfIdfEmbedding>() {
                ...
                if path.exists() { let _ = tfidf.load_vocabulary_json(&path); }
            }
        }
        if let Some(p) = manager.get_provider_mut("bagofwords") {
            if let Some(bow) = p.as_any_mut().downcast_mut::<BagOfWordsEmbedding>() {
                ...
                if path.exists() { let _ = bow.load_vocabulary_json(&path); }
            }
        }
        if let Some(p) = manager.get_provider_mut("charngram") {
            if let Some(cng) = p.as_any_mut().downcast_mut::<CharNGramEmbedding>() {
                ...
                if path.exists() { let _ = cng.load_vocabulary_json(&path); }
            }
        }
    }
        ...
        for t in &tests {
            match manager.embed_with_provider(prov, t) {
                Ok(v) => {
                    ...
                    println!("'{}' -> dim: {}, non_zero: {}, norm: {:.6}", t, dim, non_zero, norm);
                }
                Err(e) => {
                    println!("'{}' -> ERROR: {}", t, e);
                }
