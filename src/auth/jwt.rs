...
impl JwtManager {
    ...
    pub fn generate_token(&self, user_id: &str, username: &str, roles: Vec<crate::auth::roles::Role>) -> Result<String> {
    ...
    pub fn validate_token(&self, token: &str) -> Result<crate::auth::UserClaims> {
        ...
        let token_data = decode::<crate::auth::UserClaims>(token, &decoding_key, &validation)
            .map_err(|e| match e.kind() {
                ...
                _ => VectorizerError::AuthenticationError(format!("Token validation failed: {}", e)),
            })?;
