...
impl SummarizationConfig {
    ...
    pub fn from_yaml(config: &serde_yaml::Value) -> Result<Self, String> {
        ...
        if let Some(summarization) = config.get("summarization") {
            ...
            if let Some(auto_summarize) = summarization
                .get("auto_summarize")
            ...
            if let Some(summary_collection) = summarization
                .get("summary_collection")
                .and_then(|v| v.as_str())
            {
                summarization_config.summary_collection = summary_collection.to_string();
            }

            if let Some(default_method) =
                summarization.get("default_method").and_then(|v| v.as_str())
            {
            ...
            if let Some(methods) = summarization.get("methods") {
                for (method_name, method_config) in methods.as_mapping().unwrap() {
                    ...
                    if let Some(compression_ratio) = method_config
                    ...
                    if let Some(max_sentences) =
                        method_config.get("max_sentences").and_then(|v| v.as_u64())
                    {
                        config.max_sentences = Some(max_sentences as usize);
                    }

                    if let Some(min_sentence_length) = method_config
                    ...
                    if let Some(max_keywords) =
                        method_config.get("max_keywords").and_then(|v| v.as_u64())
                    {
                        config.max_keywords = Some(max_keywords as usize);
                    }

                    if let Some(min_keyword_length) = method_config
                        .get("min_keyword_length")
                        .and_then(|v| v.as_u64())
                    {
                        config.min_keyword_length = Some(min_keyword_length as usize);
                    }

                    if let Some(use_tfidf) =
                        method_config.get("use_tfidf").and_then(|v| v.as_bool())
                    {
                        config.use_tfidf = Some(use_tfidf);
                    }

                    if let Some(use_stopwords) =
                        method_config.get("use_stopwords").and_then(|v| v.as_bool())
                    {
                        config.use_stopwords = Some(use_stopwords);
                    }

                    if let Some(use_position_weight) = method_config
                        .get("use_position_weight")
                        .and_then(|v| v.as_bool())
                    {
                    ...
                    if let Some(max_tokens) =
                        method_config.get("max_tokens").and_then(|v| v.as_u64())
                    {
                        config.max_tokens = Some(max_tokens as usize);
                    }

                    if let Some(temperature) =
                        method_config.get("temperature").and_then(|v| v.as_f64())
                    {
                    ...
                    summarization_config
                        .methods
                        .insert(method_name.to_string(), config);
                }
            }
            ...
            if let Some(languages) = summarization.get("languages") {
                for (lang_code, lang_config) in languages.as_mapping().unwrap() {
                    ...
                    summarization_config
                        .languages
                        .insert(lang_code.to_string(), config);
                }
            }

            // Configurações de metadados
            if let Some(metadata) = summarization.get("metadata") {
                if let Some(include_original_id) = metadata
                    .get("include_original_id")
                    .and_then(|v| v.as_bool())
                {
                    summarization_config.metadata.include_original_id = include_original_id;
                }

                if let Some(include_file_path) =
                    metadata.get("include_file_path").and_then(|v| v.as_bool())
                {
                    summarization_config.metadata.include_file_path = include_file_path;
                }

                if let Some(include_timestamp) =
                    metadata.get("include_timestamp").and_then(|v| v.as_bool())
                {
                    summarization_config.metadata.include_timestamp = include_timestamp;
                }

                if let Some(include_method) =
                    metadata.get("include_method").and_then(|v| v.as_bool())
                {
                    summarization_config.metadata.include_method = include_method;
                }

                if let Some(include_compression_ratio) = metadata
                    .get("include_compression_ratio")
                    .and_then(|v| v.as_bool())
                {
                    summarization_config.metadata.include_compression_ratio =
                        include_compression_ratio;
                }
            }
        }
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (method_name, config) in &self.methods {
            if config.compression_ratio < 0.1 || config.compression_ratio > 0.9 {
                return Err(format!(
                    "Invalid compression ratio for method {}: {}",
                    method_name, config.compression_ratio
                ));
            }
        }
