...
mod tests {
    ...
    fn test_list_summaries_with_pagination() {
        ...
        for i in 0..5 {
            let params = SummarizationParams {
                text: format!(
                    "This is document number {} with detailed content about various topics including technology, science, and innovation. This document contains multiple paragraphs of information that need to be summarized effectively.",
                    i
                ),
                method: SummarizationMethod::Extractive,
                max_length: Some(50),
        ...
        let page1_ids: std::collections::HashSet<String> =
            page1.iter().map(|s| s.summary_id.clone()).collect();
        let page2_ids: std::collections::HashSet<String> =
            page2.iter().map(|s| s.summary_id.clone()).collect();
