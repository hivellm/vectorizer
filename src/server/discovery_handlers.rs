...
use crate::discovery::{
    Discovery, DiscoveryConfig, filter_collections, expand_queries_baseline, 
    ExpansionConfig, CollectionRef, score_collections, ScoringConfig,
    broad_discovery, BroadDiscoveryConfig, semantic_focus, SemanticFocusConfig,
    promote_readme, ReadmePromotionConfig, compress_evidence, CompressionConfig,
    build_answer_plan, AnswerPlanConfig, render_llm_prompt, PromptRenderConfig,
    ScoredChunk, ChunkMetadata, Bullet, BulletCategory, AnswerPlan, Section, SectionType,
};

// Helper to convert store collections to CollectionRef
fn get_collection_refs(store: &Arc<VectorStore>) -> Vec<CollectionRef> {
    store.list_collections().iter()
        .filter_map(|name| {
...
pub async fn handle_discover(
    ...
    let response = discovery.discover(query).await
...
pub async fn handle_filter_collections(
    ...
    let include: Vec<&str> = args.get("include")
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
    
    let exclude: Vec<&str> = args.get("exclude")
        .and_then(|v| v.as_array())
        .map(|arr| arr.iter().filter_map(|v| v.as_str()).collect())
        .unwrap_or_default();
...
pub async fn handle_broad_discovery(
    ...
    let chunks = broad_discovery(&queries, &collections, k, &config, &store, &embedding_manager).await
...
pub async fn handle_semantic_focus(
    ...
    let coll = store.get_collection(collection_name)
    ...
    let chunks = semantic_focus(&collection, &queries, k, &config, &store, &embedding_manager).await
...
pub async fn handle_promote_readme(
    ...
    let chunks_json = args.get("chunks")
        .and_then(|v| v.as_array())
...
pub async fn handle_compress_evidence(
    ...
    let max_bullets = args.get("max_bullets").and_then(|v| v.as_u64()).unwrap_or(20) as usize;
    let max_per_doc = args.get("max_per_doc").and_then(|v| v.as_u64()).unwrap_or(3) as usize;
    ...
    let bullets = compress_evidence(&chunks, max_bullets, max_per_doc, &config)
        .map_err(|e| ErrorData::internal_error(format!("Evidence compression failed: {}", e), None))?;
...
pub async fn handle_build_answer_plan(
    ...
    let bullets_json = args.get("bullets")
        .and_then(|v| v.as_array())
