...
pub async fn apply_setup_config(
    ...
    for project in &payload.projects {
        info!("📁 Processing project: {} at {}", project.name, project.path);
        ...
        for collection in &project.collections {
            ...
            if existing_collections.contains(&collection_name.to_string()) {
                info!("ℹ️  Collection {} already exists, will reindex", collection_name);
            }
            ...
            match index_project_with_loader(
                ...
                Ok(vector_count) => {
                    info!("✅ Collection {} indexed with {} vectors", collection_name, vector_count);
                ...
                Err(e) => {
                    ...
                    errors.push(format!("Failed to index collection {}: {}", collection_name, e));
                }
            }
        }
    }
    ...
    let message = if success {
        format!("Setup completed successfully. Indexed {} collections with {} vectors.", 
            collections_created, total_vectors)
    } else {
        format!("Setup completed with {} errors", errors.len())
    };
    
    info!("✅ Setup apply completed: {} collections, {} vectors, {} errors", 
        collections_created, total_vectors, errors.len());
...
async fn index_project_with_loader(
    ...
    info!("🔄 Indexing project {} into collection {}", project_path, collection_name);
...
pub fn display_first_start_guidance(host: &str, port: u16, collection_count: usize) {
    ...
    if needs_setup {
        ...
        println!("║                                                                  ║");
        println!("║  👉 {}/dashboard/setup                                       ║", base_url);
        println!("║                                                                  ║");
        println!("║  Or use the CLI:                                                 ║");
        println!("║  $ vectorizer-cli setup /path/to/your/project                    ║");
        println!("║                                                                  ║");
        println!("║  Quick links:                                                    ║");
        println!("║  • Dashboard:     {}/dashboard/overview                      ║", base_url);
        println!("║  • API Docs:      {}/dashboard/docs                          ║", base_url);
