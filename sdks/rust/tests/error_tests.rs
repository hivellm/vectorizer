...
fn test_authentication_error() {
    ...
    assert!(matches!(error, VectorizerError::Authentication { message: _ }));
...
fn test_collection_not_found_error() {
    ...
    assert!(matches!(error, VectorizerError::CollectionNotFound { ref collection } if collection == "test_collection"));
...
fn test_vector_not_found_error() {
    ...
    assert!(matches!(error, VectorizerError::VectorNotFound { ref collection, ref vector_id } 
...
fn test_configuration_error() {
    ...
    assert!(matches!(error, VectorizerError::Configuration { ref message } 
...
fn test_batch_operation_error() {
    ...
    assert!(matches!(error, VectorizerError::BatchOperation { ref message } 
...
fn test_serde_json_error_conversion() {
    let json_error = serde_json::Error::io(std::io::Error::new(std::io::ErrorKind::InvalidData, "Invalid JSON"));
...
fn test_error_display_formatting() {
    ...
    assert_eq!(format!("{}", auth_error), "Authentication failed: Invalid credentials");

    let collection_error = VectorizerError::collection_not_found("my_collection");
    assert_eq!(format!("{}", collection_error), "Collection 'my_collection' not found");

    let vector_error = VectorizerError::vector_not_found("my_collection", "vector_123");
    assert_eq!(format!("{}", vector_error), "Vector 'vector_123' not found in collection 'my_collection'");
    ...
    assert_eq!(format!("{}", server_error), "Server error: Server unavailable");
    ...
    assert_eq!(format!("{}", embedding_error), "Embedding generation failed: Embedding failed");
    ...
    assert_eq!(format!("{}", batch_error), "Batch operation failed: Batch failed");
    ...
    assert_eq!(format!("{}", serialization_error), "Serialization error: JSON failed");
}
