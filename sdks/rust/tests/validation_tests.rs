...
fn test_validate_non_empty_string() {
    ...
    assert!(
        matches!(result, Err(VectorizerError::Validation { message }) 
...
fn test_collection_name_special_characters() {
    ...
    for invalid_name in invalid_names {
        ...
        assert!(
            result.is_err(),
            "Collection name '{}' should be invalid",
            invalid_name
        );
    }
}
...
fn test_vector_id_edge_cases() {
    ...
    for valid_id in valid_ids {
        assert!(
            utils::validation::validate_vector_id(valid_id).is_ok(),
            "Vector ID '{}' should be valid",
            valid_id
        );
    }
}
...
fn test_similarity_metric_case_sensitivity() {
    ...
    for variation in case_variations {
        assert!(
            utils::validation::validate_similarity_metric(variation).is_err(),
            "Similarity metric '{}' should be invalid due to case",
            variation
        );
    }
}
