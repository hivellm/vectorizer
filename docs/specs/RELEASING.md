...
1. **`release-artifacts.yml`**
   ...
   - **Docker Build Strategy**: Uses pre-built binaries (artifact-based) instead of compiling in Docker
     ...
     - Passes pre-built artifacts to deploy/docker/Dockerfile.artifacts for final image creation
...
For releases, we use `deploy/docker/Dockerfile.artifacts` which builds from pre-compiled binaries:
...
docker buildx build -f deploy/docker/Dockerfile.artifacts --platform linux/amd64,linux/arm64 \
