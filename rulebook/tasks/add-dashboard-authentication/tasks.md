...
- [x] 1.4 Implement POST /api/dashboard/auth/logout endpoint (POST /auth/logout with token blacklist)
- [x] 1.5 Implement GET /api/dashboard/auth/verify endpoint (GET /auth/me)
- [x] 1.6 Implement POST /api/dashboard/auth/refresh endpoint (POST /auth/refresh)
- [x] 1.7 Add JWT token generation and validation (src/auth/jwt.rs)
- [x] 1.8 Add POST /auth/validate-password endpoint for password strength checking
- [ ] 1.9 Add secure cookie handling
...
- [x] 3.1 Check if any admin user exists on server startup
- [x] 3.2 Create data/ directory if not exists (uses data/auth.json)
- [x] 3.3 Create auth.json if not exists
- [x] 3.4 Generate root user if no admins found
- [x] 3.5 Support --root-user CLI argument (default: "root") + ROOT_USER env var
- [x] 3.6 Support --root-password CLI argument + ROOT_PASSWORD env var
- [x] 3.7 Generate random secure password if not provided (24 char alphanumeric+symbols)
- [x] 3.8 Print root credentials to console (one-time only, boxed banner)
- [x] 3.9 Add warning banner about changing default password
...
- [x] 5.1 Create src/server/dashboard_users.rs (in auth_handlers.rs)
- [x] 5.2 Add POST /api/dashboard/users endpoint (POST /auth/users)
- [x] 5.3 Add GET /api/dashboard/users endpoint (GET /auth/users)
- [x] 5.4 Add DELETE /api/dashboard/users/:username endpoint (DELETE /auth/users/{username})
- [x] 5.5 Add PUT /api/dashboard/users/:username/password endpoint (PUT /auth/users/{username}/password)
- [x] 5.6 Protect all user management endpoints (admin only)
- [x] 5.7 Prevent deletion of root user (cannot delete self)
- [x] 5.8 Prevent deletion of last admin user
...
- [x] 6.2 Add session storage (in-memory or Redis) (in-memory via AuthManager + token blacklist)
- [x] 6.3 Implement session expiration (default 1h)
- [x] 6.4 Add session refresh mechanism (POST /auth/refresh)
...
- [x] 8.4 Add "Remember me" checkbox (saves username to localStorage)
- [x] 8.5 Add error handling and validation (rate limiting, invalid credentials)
- [x] 8.6 Add loading states
- [x] 8.7 Style login page (modern UI)
- [x] 8.8 Add HiveLLM/Vectorizer branding
- [x] 8.9 Add password strength indicator (PasswordStrengthIndicator component)

## 9. Frontend - User Management Page

- [x] 9.1 Create dashboard/src/pages/UsersPage.tsx
- [x] 9.2 Add user list table with role and created date
- [x] 9.3 Add "Create User" button and modal
- [x] 9.4 Add create user form (username, password, role)
- [x] 9.5 Add delete user confirmation dialog
- [x] 9.6 Add change password form
- [x] 9.7 Prevent deletion of admin and self (disable button)
- [x] 9.8 Show current logged-in user indicator ("(You)")
- [x] 9.9 Add user management to dashboard navigation (admin only)

## 9A. Backend - API Key Management

- [x] 9A.1 Create src/server/dashboard_api_keys.rs (in auth_handlers.rs)
- [x] 9A.2 Implement ApiKeyStore (ApiKeyManager in src/auth/api_keys.rs)
- [x] 9A.3 Add POST /api/dashboard/api-keys endpoint (POST /auth/keys)
- [x] 9A.4 Add GET /api/dashboard/api-keys endpoint (GET /auth/keys)
- [x] 9A.5 Add DELETE /api/dashboard/api-keys/:id endpoint (DELETE /auth/keys/{id})
- [ ] 9A.6 Add PUT /api/dashboard/api-keys/:id endpoint (update permissions)
- [x] 9A.7 Generate secure random keys (format: vec_sk_32_random_chars)
- [x] 9A.8 Hash API keys with SHA-256 for storage
- [x] 9A.9 Link API keys to user account (owner_id)
- [x] 9A.10 Track last_used timestamp on each request
- [ ] 9A.11 Track usage_count for billing/monitoring
- [x] 9A.12 Support key expiration (optional)

## 9B. Frontend - API Key Management Page

- [x] 9B.1 Create dashboard/src/pages/ApiKeysPage.tsx
- [x] 9B.2 Add API keys list table (name, permissions, created, last used)
- [x] 9B.3 Add "Create API Key" button and modal
- [x] 9B.4 Add create key form (name, permissions, expiration)
- [x] 9B.5 Show generated key only once (with copy button)
- [x] 9B.6 Add warning: "Save this key - it won't be shown again"
- [x] 9B.7 Add revoke key confirmation dialog
- [ ] 9B.8 Add key usage statistics (total requests)
- [x] 9B.9 Add permissions badges (read/write/admin)
- [x] 9B.10 Add "API Keys" to dashboard navigation
...
- [x] 10.4 Add logout() function (calls server /auth/logout to invalidate token)
- [x] 10.5 Add verifySession() function
- [x] 10.6 Add auto-refresh session logic (refreshToken() + 50-min interval)
...
- [x] 15.1 Add --root-user argument to CLI
- [x] 15.2 Add --root-password argument to CLI
- [x] 15.3 Support ROOT_USER environment variable
- [x] 15.4 Support ROOT_PASSWORD environment variable
- [ ] 15.5 Validate password complexity on CLI input
- [x] 15.6 Print root user credentials on first startup (boxed banner)
- [x] 15.7 Add warning about changing default password
...
- [x] 18.1 Implement rate limiting on login endpoint (5 attempts/5 minutes)
- [x] 18.2 Add brute-force protection (account lockout 15min after 5 failures)
- [ ] 18.3 Implement CSRF protection
- [x] 18.4 Add security headers (CSP, X-Frame-Options, X-Content-Type-Options, etc.)
- [ ] 18.5 Use HTTPS in production (enforce TLS)
- [ ] 18.6 Audit log all login attempts with IP
- [x] 18.7 Password complexity requirements (8+ chars, uppercase, lowercase, number)
