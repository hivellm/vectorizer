...
fn convert_qdrant_hnsw_config(
    ...
    current_config: &vectorizer::models::HnswConfig,
...
fn convert_to_qdrant_config(
    config: &vectorizer::models::CollectionConfig,
) -> QdrantCollectionConfig {
    let distance = match config.metric {
        vectorizer::models::DistanceMetric::Cosine => QdrantDistance::Cosine,
        vectorizer::models::DistanceMetric::Euclidean => QdrantDistance::Euclid,
        vectorizer::models::DistanceMetric::DotProduct => QdrantDistance::Dot,
    };
    ...
    let quantization_config = match &config.quantization {
        vectorizer::models::QuantizationConfig::SQ { bits } => Some(
            QdrantQuantizationConfig::Scalar(QdrantScalarQuantizationConfig {
        ...
        vectorizer::models::QuantizationConfig::PQ {
            n_centroids,
            n_subquantizers,
        } => {
        ...
        vectorizer::models::QuantizationConfig::Binary => Some(QdrantQuantizationConfig::Binary(
            QdrantBinaryQuantizationConfig {
                binary: QdrantBinaryQuantization { always_ram: None },
            },
        )),
        vectorizer::models::QuantizationConfig::None => None,
    };
...
fn convert_from_qdrant_config(
    ...
    let metric = match request.config.vectors.distance {
        QdrantDistance::Cosine => vectorizer::models::DistanceMetric::Cosine,
        QdrantDistance::Euclid => vectorizer::models::DistanceMetric::Euclidean,
    ...
    let quantization_config = if let Some(quantization) = &request.config.quantization_config {
        match quantization {
            ...
            QdrantQuantizationConfig::Binary(_) => vectorizer::models::QuantizationConfig::Binary,
        }
    } else {
        vectorizer::models::QuantizationConfig::SQ { bits: 8 } // Default to 8-bit quantization
    };

    Ok(vectorizer::models::CollectionConfig {
        dimension,
        metric,
        hnsw_config,
