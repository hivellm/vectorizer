...
//!   [`vectorizer::file_watcher::FileWatcherConfig`], falling back to
...
-> anyhow::Result<vectorizer::file_watcher::FileWatcherConfig> {
    match vectorizer::file_watcher::FileWatcherConfig::from_yaml_file("workspace.yml") {
        Ok(config) => {
            info!(
        ...
        Err(e) => {
            ...
            Ok(vectorizer::file_watcher::FileWatcherConfig::default())
        }
    }
}
...
/// [`vectorizer::file_loader::FileLoader`].
pub(super) async fn load_workspace_collections(
    ...
    use vectorizer::workspace::manager::WorkspaceManager;
                            ...
                            match reader.read_file(&vector_store_path) {
                                Ok(data) => {
                                    ...
                                    let persisted = match serde_json::from_slice::<
                                        vectorizer::persistence::PersistedVectorStore,
                                    >(
                                        &data
                                    ) {
                                        Ok(persisted_store) => {
                                        ...
                                        Err(_) => {
                                            ...
                                            serde_json::from_slice::<
                                                vectorizer::persistence::PersistedCollection,
                                            >(&data)
                                            .ok()
                                        }
                                    };
                                    ...
                                    if let Some(mut persisted) = persisted {
                                        ...
                                        let vectors: Vec<vectorizer::models::Vector> = persisted
                                            .vectors
                                            .into_iter()
