...
pub async fn handle_umicp_request(state: UmicpState, envelope: Envelope) -> Result<Envelope> {
    ...
    let caps = envelope.capabilities().ok_or_else(|| {
        vectorizer_core::error::VectorizerError::Other("Missing capabilities".to_string())
    })?;
    ...
    let operation = caps
        ...
        .and_then(|v| v.as_str())
        .ok_or_else(|| {
            vectorizer_core::error::VectorizerError::Other(
                "Missing 'operation' in capabilities".to_string(),
            )
        })?;
...
fn content_to_json(content: &[Content]) -> serde_json::Value {
    ...
    serde_json::from_str(&content_json).unwrap_or_else(|_| json!({"content": content_json}))
}

/// Create success response envelope
/// Now with native JSON types (v0.2.1)
fn create_success_response(request: Envelope, result: Value) -> Result<Envelope> {
    ...
    response_caps.insert(
        "original_message_id".to_string(),
        json!(request.message_id()),
    );
...
/// Now with native JSON types (v0.2.1)
fn create_error_response(request: Envelope, error_message: &str) -> Result<Envelope> {
    let mut response_caps = HashMap::new();
