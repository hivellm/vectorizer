...
impl MutationRoot {
    ...
    async fn create_collection(
        ...
        if let (Some(tenant), Some(quota_mgr)) = (tenant_ctx, &gql_ctx.quota_manager) {
            ...
            match quota_mgr
                .check_quota(
                    &tenant.tenant_id,
                    vectorizer::hub::quota::QuotaType::CollectionCount,
                    1,
                )
                .await
            {
        ...
        let mut config = CollectionConfig {
            ...
            metric: input
                .metric
                .map(|m| m.into())
                .unwrap_or(vectorizer::models::DistanceMetric::Cosine),
        ...
        if let Some(shard_count) = input.shard_count {
            config.sharding = Some(vectorizer::models::ShardingConfig {
        ...
        if enable_graph {
            config.graph = Some(vectorizer::models::GraphConfig::default());
        }
    ...
    async fn upsert_vector(
        ...
        if let (Some(tenant), Some(quota_mgr)) = (tenant_ctx, &gql_ctx.quota_manager) {
            match quota_mgr
                .check_quota(
                    &tenant.tenant_id,
                    vectorizer::hub::quota::QuotaType::VectorCount,
                    1,
                )
                .await
            {
        ...
        let payload = if let Some(payload_json) = input.payload {
            if let Some(ref key) = input.public_key {
                // Encrypt payload
                let encrypted =
                    vectorizer::security::payload_encryption::encrypt_payload(&payload_json.0, key)
                        .map_err(|e| {
    ...
    async fn update_payload(
        ...
        let new_payload = if let Some(ref key) = public_key {
            let encrypted =
                vectorizer::security::payload_encryption::encrypt_payload(&payload.0, key)
                    .map_err(|e| {
                        async_graphql::Error::new(format!("Failed to encrypt payload: {e}"))
                    })?;
    ...
    async fn add_workspace(
        ...
        let workspace_manager = vectorizer::config::WorkspaceManager::new();
    ...
    async fn upload_file(
        ...
        for chunk in &chunks {
            ...
            let mut payload = if let Some(ref key) = input.public_key {
                match vectorizer::security::payload_encryption::encrypt_payload(&payload_data, key)
                {
