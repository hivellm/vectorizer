...
async fn add_node(
    ...
    let mut node = vectorizer::cluster::ClusterNode::new(
        node_id.clone(),
...
async fn trigger_rebalance(
    ...
    let active_nodes: Vec<_> = nodes
        .iter()
        .filter(|n| matches!(n.status, vectorizer::cluster::NodeStatus::Active))
        .collect();
    ...
    // Collect all shards
    let all_shards: Vec<vectorizer::db::sharding::ShardId> = current_distribution
        .values()
...
fn get_current_shard_distribution(
    shard_router: &std::sync::Arc<vectorizer::cluster::DistributedShardRouter>,
    nodes: &[vectorizer::cluster::ClusterNode],
) -> std::collections::HashMap<String, Vec<vectorizer::db::sharding::ShardId>> {
