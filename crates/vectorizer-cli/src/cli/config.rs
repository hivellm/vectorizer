...
pub struct AuthConfigFile {
    ...
    pub jwt_secret: vectorizer::auth::Secret<String>,
    ...
    fn default() -> Self {
        Self {
            ...
            auth: AuthConfigFile {
                ...
                // See `vectorizer::auth::LEGACY_INSECURE_DEFAULT_SECRET` for the
                // historical value rejected by validation.
                jwt_secret: vectorizer::auth::Secret::new(String::new()),
                jwt_expiration: 3600,
                api_key_length: 32,
...
impl ConfigManager {
    ...
    pub fn validate_config(config: &ConfigFile) -> Result<()> {
        ...
        if config.auth.enabled {
            ...
            if jwt_secret == vectorizer::auth::LEGACY_INSECURE_DEFAULT_SECRET {
                return Err(VectorizerError::InvalidConfiguration {
            ...
            if jwt_secret.len() < vectorizer::auth::MIN_JWT_SECRET_LEN {
                return Err(VectorizerError::InvalidConfiguration {
                    message: format!(
                        "JWT secret must be at least {} characters long",
                        vectorizer::auth::MIN_JWT_SECRET_LEN
                    ),
                });
            }
        }
    ...
    fn convert_to_cli_config(config_file: ConfigFile) -> CliConfig {
        CliConfig {
            ...
            auth: vectorizer::auth::AuthConfig {
...
mod tests {
    ...
    fn test_config_validation() {
        ...
        config.auth.jwt_secret = vectorizer::auth::Secret::new("v".repeat(64));
        ...
        config.auth.jwt_secret = vectorizer::auth::Secret::new("short".to_string());
        assert!(ConfigManager::validate_config(&config).is_err());

        // Legacy insecure default must be explicitly rejected (not merely
        // because it happens to satisfy the length check).
        config.auth.jwt_secret = vectorizer::auth::Secret::new(
            vectorizer::auth::LEGACY_INSECURE_DEFAULT_SECRET.to_string(),
        );
        assert!(ConfigManager::validate_config(&config).is_err());
    ...
    fn test_config_file_operations() {
        ...
        default_file.auth.jwt_secret = vectorizer::auth::Secret::new("o".repeat(64));
