...
async fn main() {
    ...
    match args.command {
        ...
        Commands::Setup { path, wizard } => {
            if wizard {
                if let Err(e) = vectorizer_cli::cli::setup::run_wizard().await {
                    error!("Setup wizard failed: {e}");
                    std::process::exit(1);
                }
            } else if let Err(e) = vectorizer_cli::cli::setup::run(path).await {
                error!("Setup failed: {e}");
                std::process::exit(1);
            }
        }
        Commands::Docs { sandbox } => {
            if let Err(e) = vectorizer_cli::cli::setup::run_docs(sandbox).await {
        ...
        Commands::Cli => {
            // Run legacy CLI
            if let Err(e) = vectorizer_cli::cli::run().await {
                error!("Error: {e}");
                std::process::exit(1);
            }
        }
