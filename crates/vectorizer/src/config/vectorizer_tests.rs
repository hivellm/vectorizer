...
// =========================================================================
// BackpressureConfig Tests (issue #263)
// =========================================================================
...
#[test]
fn test_backpressure_resolved_max_zero_uses_num_cpus() {
    let config = BackpressureConfig::default();
...
#[test]
fn test_backpressure_resolved_max_explicit_value_wins() {
...
#[test]
fn test_backpressure_validate_ok_for_default() {
    BackpressureConfig::default()
        .validate()
...
#[test]
fn test_backpressure_validate_rejects_inverted_watermarks() {
    let config = BackpressureConfig {
...
#[test]
fn test_backpressure_validate_rejects_zero_hard_limit() {
    let config = BackpressureConfig {
...
#[test]
fn test_backpressure_validate_rejects_zero_retry_after() {
    let config = BackpressureConfig {
...
#[test]
fn test_backpressure_yaml_round_trip() {
    let config = BackpressureConfig {
        enabled: true,
...
#[test]
fn test_backpressure_partial_yaml_uses_defaults() {
    // Operator only overrides one field — everything else stays at the
    // proposal defaults. This guarantees forward-compat for existing
...
#[test]
fn test_backpressure_in_vectorizer_config() {
    let config = VectorizerConfig::default();
...
#[test]
fn test_vectorizer_config_validate_propagates_backpressure_error() {
...
#[test]
fn test_backpressure_env_override_max_concurrent() {
    // SAFETY: env mutation is only safe in tests if no other test in
    // the same binary races on the same variable. We use a unique
    // per-test name so this remains isolated even under cargo's default
...
#[test]
fn test_backpressure_env_override_enabled_falsey() {
