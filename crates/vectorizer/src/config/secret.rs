...
//!
//! Owned by `config` (not `auth`) so that config-owned types (e.g.
//! `AuthConfig::jwt_secret`) can reference it without creating a
//! `config -> auth` dependency edge. `crate::auth::Secret` re-exports
