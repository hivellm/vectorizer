...
impl ContextFormatter {
    ...
    pub fn format_context(&self, results: &[IntelligentSearchResult], query: &str) -> String {
        if results.is_empty() {
            return String::new();
        }
    ...
    fn format_single_result(&self, result: &IntelligentSearchResult, query: &str) -> String {
        let mut parts = Vec::new();

        // Add header with collection and score
        let header = format!("[{}] (score: {:.3})", result.collection, result.score);
        parts.push(header);
...
mod tests {
    ...
    fn test_format_single_result() {
        let formatter = ContextFormatter::default();
        let result =
            create_test_result("vectorizer is a vector database for semantic search", 0.85);
    ...
    fn test_format_enhanced_context() {
        let formatter = ContextFormatter::default();
        let results = vec![create_test_result("vectorizer is a vector database", 0.8)];

        let enhanced =
            formatter.format_enhanced_context(&results, "vectorizer", Some("Found 1 result"));
