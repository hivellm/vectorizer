...
// Tenant model types moved to `models/tenant.rs` in phase41 (cluster
// needed TenantContext and cluster → hub was an upward back-reference).
// Re-exported here so every existing `crate::hub::...` import keeps
// working.
pub use crate::models::tenant::{TenantContext, TenantPermission, TenantRateLimits};

/// SDK-permission conversion. Lives here (not in `models/tenant.rs`)
/// because it depends on the HiveHub SDK type — the models layer must
/// stay free of service/SDK imports. Inherent impls may live in any
/// module of the defining crate, so call sites keep using
/// `TenantPermission::from_sdk_permission(..)` unchanged.
