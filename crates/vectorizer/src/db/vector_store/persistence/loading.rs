//! Collection loading — compact `.vecdb` archive, legacy raw files,
//! HNSW cache-path helpers, and dynamic collections.
//!
...
impl VectorStore {
    ...
    pub(in crate::db::vector_store) fn load_persisted_collection<P: AsRef<std::path::Path>>(
        &self,
        path: P,
        collection_name: &str,
    ) -> Result<()> {
