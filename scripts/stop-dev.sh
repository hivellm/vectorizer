...
# Vectorizer Development Stop Script
# Stops all development servers (cargo run processes)

echo "🛑 Stopping Vectorizer Development Servers..."
echo "=============================================="

# Find and kill vzr orchestrator processes (cargo run)
VZR_PIDS=$(pgrep -f "cargo.*run.*--bin.*vzr" || true)
if [ ! -z "$VZR_PIDS" ]; then
    echo "Stopping vzr orchestrator (development) (PIDs: $VZR_PIDS)"
    echo "$VZR_PIDS" | xargs kill 2>/dev/null || true
    echo "✅ vzr orchestrator stopped"
else
    echo "ℹ️  No vzr orchestrator (development) running"
fi

# Find and kill vectorizer-mcp-server processes (cargo run)
MCP_PIDS=$(pgrep -f "cargo.*run.*--bin.*vectorizer-mcp-server" || true)
if [ ! -z "$MCP_PIDS" ]; then
    echo "Stopping MCP servers (development) (PIDs: $MCP_PIDS)"
    echo "$MCP_PIDS" | xargs kill 2>/dev/null || true
    echo "✅ MCP servers stopped"
else
    echo "ℹ️  No MCP servers (development) running"
fi

# Find and kill vectorizer-server processes (cargo run)
REST_PIDS=$(pgrep -f "cargo.*run.*--bin.*vectorizer-server" || true)
if [ ! -z "$REST_PIDS" ]; then
    echo "Stopping REST servers (development) (PIDs: $REST_PIDS)"
    echo "$REST_PIDS" | xargs kill 2>/dev/null || true
    echo "✅ REST servers stopped"
else
    echo "ℹ️  No REST servers (development) running"
fi
