...
use vectorizer::file_loader::{DocumentChunk, LoaderConfig};
...
impl BinaryQuantizer {
    ...
    pub fn train(&mut self, vectors: &[Vec<f32>]) {
        ...
        let total: f32 = vectors.iter().flat_map(|v| v.iter().copied()).sum();
...
/// Create mock documents for benchmark purposes
fn create_mock_documents(path: &str, count: usize) -> Vec<DocumentChunk> {
    let mut docs = Vec::new();
    for i in 0..count {
        docs.push(DocumentChunk {
            id: format!("doc_{}_{}", path.replace('/', "_"), i),
            content: format!("Mock document content for {} - chunk {}", path, i),
            file_path: format!("{}/mock_file_{}.txt", path, i),
            chunk_index: i,
...
async fn load_massive_dataset(
    ...
    for path in &workspace_paths {
        ...
        let config = LoaderConfig {
            ...
            collection_name: "large_scale_benchmark".to_string(),
...
fn benchmark_at_scale(
    ...
    let batch_vectors: Vec<_> = vector_ids
        .iter()
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    md.push_str(&format!(
        "# Large Scale Benchmark - {}K Vectors\n\n",
        vector_count / 1000
    ));
    md.push_str(&format!(
        "**Generated**: {}\n\n",
    ...
    md.push_str(
        "| Method | Memory | Compression | Build Time | Search (ms) | QPS | MAP | Recall@10 |\n",
    );
    md.push_str(
        "|--------|--------|-------------|------------|-------------|-----|-----|----------|\n",
    );
    ...
    if let Some(baseline) = results.iter().find(|r| r.quantization == "Baseline") {
        ...
        for result in &results {
            ...
            md.push_str(&format!(
                "- **Search Speed**: {:.2}ms ({:.1}% vs baseline)\n\n",
                result.avg_search_latency_ms,
    ...
    if let Some(baseline) = results.iter().find(|r| r.quantization == "Baseline") {
        ...
        if let Some(best) = best_balanced {
            md.push_str(&format!(
                "**Best Balanced**: {} (≥95% quality retention)\n",
                best.quantization
            ));
    ...
    let md_path = report_dir.join(format!(
        "large_scale_{}k_{}.md",
        vector_count / 1000,
        timestamp
    ));
