...
impl TestDataset {
    fn load_from_workspace(max_documents: usize) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        for (project_name, project_path) in &workspace_paths {
            ...
            let mut loader = FileLoader::with_embedding_manager(config, embedding_manager);

            match loader
                .load_and_index_project(project_path, &temp_store)
                .await
            {
                Ok(chunk_count) => {
...
fn load_test_dataset() -> TestDataset {
    TestDataset::load_from_workspace(1000).expect("Failed to load test dataset")
}
...
fn bench_baseline(c: &mut Criterion) {
    ...
    c.bench_function("quantization_baseline", |b| {
        b.iter(|| black_box(benchmark_baseline(&dataset, dimension).unwrap()))
    });
}

/// Benchmark scalar quantization with different bit depths
fn bench_scalar_quantization(c: &mut Criterion) {
    ...
    for bits in [4, 8] {
        group.bench_with_input(BenchmarkId::new("sq", bits), &bits, |b, &bits| {
            b.iter(|| black_box(benchmark_sq(&dataset, dimension, bits).unwrap()))
        });
    }
...
/// Benchmark product quantization with different configurations
fn bench_product_quantization(c: &mut Criterion) {
    ...
    let configs = [(8, 256), (16, 256), (8, 512)];

    for (subquantizers, centroids) in configs {
        group.bench_with_input(
            BenchmarkId::new("pq", format!("{}x{}", subquantizers, centroids)),
            &(subquantizers, centroids),
            |b, &(subquantizers, centroids)| {
                b.iter(|| {
                    black_box(benchmark_pq(&dataset, dimension, subquantizers, centroids).unwrap())
                })
            },
        );
    }
...
fn bench_binary_quantization(c: &mut Criterion) {
    ...
    c.bench_function("binary_quantization", |b| {
        b.iter(|| black_box(benchmark_binary(&dataset, dimension).unwrap()))
    });
}

/// Benchmark search performance for different quantization methods
fn bench_search_performance(c: &mut Criterion) {
    ...
    let methods: Vec<(&str, Box<dyn Fn(&TestDataset, usize) -> f64 + Send + Sync>)> = vec![
        (
            "baseline",
            Box::new(|dataset: &TestDataset, dim: usize| benchmark_baseline(dataset, dim).unwrap()),
        ),
        (
            "sq_8bit",
            Box::new(|dataset: &TestDataset, dim: usize| benchmark_sq(dataset, dim, 8).unwrap()),
        ),
        (
            "pq_8x256",
            Box::new(|dataset: &TestDataset, dim: usize| {
                benchmark_pq(dataset, dim, 8, 256).unwrap()
            }),
        ),
        (
            "binary",
            Box::new(|dataset: &TestDataset, dim: usize| benchmark_binary(dataset, dim).unwrap()),
        ),
    ];

    for (method_name, benchmark_fn) in methods {
        group.bench_function(method_name, |b| {
            b.iter(|| black_box(benchmark_fn(&dataset, dimension)))
        });
    }
