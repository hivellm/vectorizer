...
fn benchmark_insert_single(
    ...
    for (id, vec) in test_vectors.iter().take(1000) { // Test with 1000 inserts
    ...
    Ok(OperationMetrics {
        ...
        min_latency_us: latencies.iter().copied().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap(),
        max_latency_us: latencies.iter().copied().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap(),
        ...
        memory_delta_mb: (mem_after.total_memory_bytes - mem_before.total_memory_bytes) as f64 / 1_048_576.0,
    })
}
...
fn benchmark_insert_batch(
    ...
    for &batch_size in batch_sizes {
        ...
        // Insert in batches
        for batch in test_vectors.chunks(batch_size).take(10) { // 10 batches
            let start = Instant::now();
    ...
    Ok(OperationMetrics {
        ...
        min_latency_us: all_latencies.iter().copied().min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap(),
        max_latency_us: all_latencies.iter().copied().max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap(),
...
fn benchmark_update_batch(
    ...
    for batch_idx in 0..10 { // 10 batches of 100
        ...
        println!("    Updated batch {}/10 ({} vectors)", batch_idx + 1, batch_size);
    }
...
fn benchmark_delete_batch(
    ...
    for batch_idx in 0..10 { // 10 batches
        let start_idx = (batch_idx * batch_size) % test_vectors.len();
        ...
        println!("    Deleted batch {}/10 ({} vectors)", batch_idx + 1, batch_size);
    }
...
fn benchmark_concurrent_mixed(
    ...
    let all_latencies: Vec<f64> = operation_times.values()
...
fn generate_report(report: &BenchmarkReport) -> String {
    ...
    for op in &ops {
        ...
        md.push_str(&format!("- **Total Time**: {:.2} ms ({:.2} s)\n", op.total_time_ms, op.total_time_ms / 1000.0));
        md.push_str(&format!("- **Throughput**: {:.2} ops/sec\n", op.throughput_ops_per_sec));
        md.push_str(&format!("- **Throughput**: {:.2} ops/min\n\n", op.throughput_ops_per_sec * 60.0));
        
        md.push_str("#### Latency Distribution\n\n");
        md.push_str(&format!("- **Average**: {:.0} μs ({:.2} ms)\n", op.avg_latency_us, op.avg_latency_us / 1000.0));
        md.push_str(&format!("- **P50 (Median)**: {:.0} μs\n", op.p50_latency_us));
    ...
    let batch_speedup = report.insert_batch.throughput_ops_per_sec / report.insert_single.throughput_ops_per_sec;
    md.push_str(&format!(
    ...
    md.push_str(&format!(
        ...
        report.insert_single.throughput_ops_per_sec,
        report.insert_single.avg_latency_us
    ));
    md.push_str(&format!(
        "- Batch insert: {:.2} ops/sec ({:.0} μs/batch)\n\n",
        report.insert_batch.throughput_ops_per_sec,
        report.insert_batch.avg_latency_us
    ));
    
    md.push_str("### Search Performance\n\n");
    md.push_str(&format!(
        "- **k=1**: {:.0} μs avg, {:.2} QPS\n",
        report.search_k1.avg_latency_us,
        report.search_k1.throughput_ops_per_sec
    ));
    md.push_str(&format!(
        "- **k=10**: {:.0} μs avg, {:.2} QPS\n",
        report.search_k10.avg_latency_us,
        report.search_k10.throughput_ops_per_sec
    ));
    md.push_str(&format!(
        "- **k=100**: {:.0} μs avg, {:.2} QPS\n",
        report.search_k100.avg_latency_us,
        report.search_k100.throughput_ops_per_sec
    ));
    md.push_str(&format!(
    ...
    let update_speedup = report.update_batch.throughput_ops_per_sec / report.update_single.throughput_ops_per_sec;
    md.push_str(&format!(
    ...
    let delete_speedup = report.delete_batch.throughput_ops_per_sec / report.delete_single.throughput_ops_per_sec;
    md.push_str(&format!(
    ...
    let targets_met = vec![
        ("Insert < 100μs/op (batch)", report.insert_batch.avg_latency_us < 100.0 * 1000.0),
        ("Search < 5ms (k=10)", report.search_k10.avg_latency_us < 5000.0),
        ("Update < 200μs/op (batch)", report.update_batch.avg_latency_us < 200.0 * 1000.0),
        ("Delete < 50μs/op (batch)", report.delete_batch.avg_latency_us < 50.0 * 1000.0),
    ];
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  ✅ Throughput: {:.2} ops/sec", insert_single.throughput_ops_per_sec);
    println!("  ✅ Latency: {:.0} μs (p95: {:.0} μs)", 
    ...
    println!("  ✅ Throughput: {:.2} ops/sec", concurrent_mixed.throughput_ops_per_sec);
    println!("  ✅ Latency: {:.0} μs (p95: {:.0} μs)", 
    ...
    let insert_speedup = report.insert_batch.throughput_ops_per_sec / report.insert_single.throughput_ops_per_sec;
    let update_speedup = report.update_batch.throughput_ops_per_sec / report.update_single.throughput_ops_per_sec;
    let delete_speedup = report.delete_batch.throughput_ops_per_sec / report.delete_single.throughput_ops_per_sec;
