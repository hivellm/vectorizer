...
async fn run_single_benchmark(test_name: &str, vector_count: usize, query_count: usize) -> Result<BenchmarkResults> {
    ...
    tracing::info!("    HNSW: {:.2}ms avg ({:.2}ms total)", hnsw_avg, hnsw_duration);
    if cuda_duration > 0.0 {
        tracing::info!("    CUDA: {:.2}ms avg ({:.2}ms total) - {:.2}x speedup", cuda_avg, cuda_duration, cuda_speedup);
    } else {
...
fn print_benchmark_summary(results: &[BenchmarkResults]) {
    tracing::info!("\n📊 BENCHMARK SUMMARY");
    tracing::info!("===================");
    tracing::info!("{:<20} {:<10} {:<10} {:<10} {:<12} {:<10}",
    ...
    for result in results {
        ...
        tracing::info!("{:<20} {:<10} {:<10} {:<10.2} {:<12} {:<10}",
                 result.test_name,
                 result.vector_count,
                 result.query_count,
