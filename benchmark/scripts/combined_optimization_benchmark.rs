        ...
        queries.iter().enumerate().map(|(query_idx, query)| {
            ...
            if relevant.is_empty() {
                ...
                for (idx, doc) in docs.iter().enumerate() {
                    ...
                    if matching >= 1 { // Lower threshold for fallback
                        relevant.insert(format!("doc_{}", idx));
                    }
                }
            }
...
fn benchmark_config(
    ...
    let (final_vectors, memory_quantized_mb, quant_config) = match quantization {
        "None" => {
            (vectors.clone(), memory_original_mb, "No quantization".to_string())
        }
        ...
        "PQ" => {
            ...
            (decoded, mem, format!("PQ {}x{}", n_subquantizers, n_centroids))
        }
        ...
        "Binary" => {
            ...
            let decoded: Vec<Vec<f32>> = encoded.iter().map(|c| binary.decode(c, dimension)).collect();
    ...
    let batch_vectors: Vec<(String, Vec<f32>)> = vector_ids.iter()
    ...
    let overall_score = quality_score * 0.5 + efficiency_score * 0.1 + (speed_normalized / 2.0) * 0.2;
    
    println!("    Memory: {:.2} MB ({:.1}x compression)", memory_quantized_mb, compression_ratio);
...
fn generate_report(results: &[ConfigBenchmark], dataset_size: usize) -> String {
    ...
    let best_memory = results.iter()
        .min_by(|a, b| a.memory_quantized_mb.partial_cmp(&b.memory_quantized_mb).unwrap())
        .unwrap();
    
    let best_speed = results.iter()
        .min_by(|a, b| a.avg_search_latency_us.partial_cmp(&b.avg_search_latency_us).unwrap())
        .unwrap();
    ...
    for dimension in [256, 384, 512, 768, 1024] {
        ...
        let dim_results: Vec<_> = results.iter()
    ...
    for dimension in [256, 384, 512, 768, 1024] {
        ...
        if let Some(base) = baseline {
            ...
            for quant in ["SQ-8bit", "PQ", "Binary"] {
                if let Some(result) = results.iter().find(|r| r.dimension == dimension && r.quantization == quant) {
    ...
    for result in &sorted_results {
        let mem_saving = ((baseline_512.memory_quantized_mb - result.memory_quantized_mb) / baseline_512.memory_quantized_mb) * 100.0;
    ...
    md.push_str("Reasons:\n");
    md.push_str(&format!("- Quality: {:.4} MAP (best balance)\n", sorted_results[0].map));
    md.push_str(&format!("- Memory: {:.2} MB ({:.1}x compression)\n", 
        sorted_results[0].memory_quantized_mb, 
        sorted_results[0].compression_ratio));
    md.push_str(&format!("- Performance: {:.0} μs ({:.0} QPS)\n", 
        sorted_results[0].avg_search_latency_us,
        sorted_results[0].search_throughput_qps));
    ...
    md.push_str(&format!("   - {:.2} MB memory, {:.4} MAP\n\n", 
    ...
    let memory_constrained = results.iter()
    ...
    if let Some(config) = low_latency {
        md.push_str(&format!("   - {}D + {}\n", config.dimension, config.quantization));
        md.push_str(&format!("   - {:.0} μs latency, {:.4} MAP\n\n", 
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📊 Dataset: {} documents, {} queries", 
        dataset.documents.len(), dataset.queries.len());
    
    let dimensions = vec![256, 384, 512, 768, 1024];
    let quantizations = vec!["None", "SQ-8bit", "PQ", "Binary"];
    
    println!("\n🧪 Testing {} configurations...", dimensions.len() * quantizations.len());
    ...
    for &dimension in &dimensions {
        for quantization in &quantizations {
            completed += 1;
            println!("\n[{}/{}] Testing {}D + {}...", completed, total, dimension, quantization);
    ...
    println!();
    println!("Benefits:");
    println!("  ✅ Quality: {:.4} MAP ({:.1}% of best possible)", 
        sorted[0].map,
        (sorted[0].map / sorted.iter().map(|r| r.map).max_by(|a, b| a.partial_cmp(b).unwrap()).unwrap()) * 100.0);
    println!("  ✅ Memory: {:.2} MB ({:.1}x compression)", 
        sorted[0].memory_quantized_mb, 
        sorted[0].compression_ratio);
    println!("  ✅ Speed: {:.0} μs ({:.0} QPS)", 
        sorted[0].avg_search_latency_us,
        sorted[0].search_throughput_qps);
    println!("  ✅ Efficiency: {:.4} MAP/MB", sorted[0].efficiency_score);
    
    // Comparison to current default (512D + None)
    if let Some(current_default) = results.iter().find(|r| r.dimension == 512 && r.quantization == "None") {
        let mem_improvement = ((current_default.memory_quantized_mb - sorted[0].memory_quantized_mb) / current_default.memory_quantized_mb) * 100.0;
        let quality_improvement = ((sorted[0].map / current_default.map) - 1.0) * 100.0;
        let speed_improvement = ((current_default.avg_search_latency_us - sorted[0].avg_search_latency_us) / current_default.avg_search_latency_us) * 100.0;
