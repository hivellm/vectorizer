...
fn benchmark_dimension(
    ...
    println!("    Generated {} embeddings in {:.2}s", 
        vectors.len(), embed_time.as_secs_f64());
    ...
    println!("    Memory: {:.2} MB ({:.0} bytes/vector)", memory_mb, memory_per_vector);
...
fn generate_report(
    ...
    let reference = results.iter().find(|r| r.dimension == 512)
    ...
    for result in results {
        ...
        let recall_diff = ((result.quality_recall_at_10 / reference.quality_recall_at_10) - 1.0) * 100.0;
    ...
    md.push_str(&format!("- Memory cost: {:.2} MB\n\n", best_quality.memory_mb));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  ⚡ Most Efficient: {}D ({:.4} MAP/MB)", 
    ...
    for result in &results {
        ...
        let memory_saving = ((reference.memory_mb - result.memory_mb) / reference.memory_mb) * 100.0;
        
        if result.dimension < 512 {
            println!("     {}D: {:.1}% quality, {:.1}% less memory",
                result.dimension, retention, memory_saving);
        } else if result.dimension > 512 {
            let memory_cost = ((result.memory_mb - reference.memory_mb) / reference.memory_mb) * 100.0;
            println!("     {}D: {:.1}% quality, {:+.1}% more memory",
    ...
    let mut scored: Vec<_> = results.iter()
        .map(|r| {
            let quality_score = r.quality_map / best_quality.quality_map;
            let speed_score = best_speed.avg_search_latency_us / r.avg_search_latency_us;
            let memory_score = results.iter().map(|x| x.memory_mb).min_by(|a, b| a.partial_cmp(b).unwrap()).unwrap() / r.memory_mb;
    ...
    println!("     - Quality: {:.1}% of best", 
        (scored[0].0.quality_map / best_quality.quality_map) * 100.0);
    println!("     - Speed: {:.0} μs avg", scored[0].0.avg_search_latency_us);
