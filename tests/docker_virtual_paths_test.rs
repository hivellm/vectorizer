...
use vectorizer::file_operations::{FileOperations, FileOperationError};
...
async fn test_docker_virtual_paths_acceptance() {
    ...
    for path in docker_virtual_paths {
        ...
        let result = ops
            .get_file_content("test-collection", path, 100)
            .await;
        ...
        match result {
            Err(FileOperationError::InvalidPath { .. }) => {
                panic!("Path '{}' should be valid but was rejected as invalid", path);
            }
            ...
            Err(FileOperationError::FileNotFound { .. }) => {
                ...
                println!("✓ Path '{}' passed validation (failed later as expected)", path);
            }
            Err(e) => {
...
async fn test_only_empty_paths_rejected() {
    ...
    for path in empty_paths {
        ...
        match result {
            ...
            _ => {
                panic!("Empty path '{}' should be rejected", 
                    path.replace('\n', "\\n").replace('\t', "\\t"));
            }
        }
    }
