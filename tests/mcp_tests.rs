...
use vectorizer::mcp::{McpConfig, McpConnection, McpRequestMessage, McpServerState};
// use std::sync::Arc;
use serde_json::json;
...
async fn test_mcp_config_serialization() {
    ...
    let deserialized: McpConfig =
        serde_json::from_str(&json).expect("Failed to deserialize MCP config");
...
async fn test_mcp_request_serialization() {
    ...
    let deserialized: McpRequest =
        serde_json::from_str(&json).expect("Failed to deserialize MCP request");
...
async fn test_mcp_response_serialization() {
    ...
    let deserialized: McpResponse =
        serde_json::from_str(&json).expect("Failed to deserialize MCP response");
...
async fn test_mcp_error_handling() {
    ...
    let deserialized: McpError =
        serde_json::from_str(&json).expect("Failed to deserialize MCP error");
...
async fn test_mcp_tool_call_serialization() {
    ...
    let deserialized: ToolCall =
        serde_json::from_str(&json).expect("Failed to deserialize tool call");
...
async fn test_mcp_chat_request_serialization() {
    ...
    let deserialized: McpRequestMessage =
...
async fn test_mcp_embedding_request_serialization() {
    ...
    let deserialized: McpRequestMessage =
        serde_json::from_str(&json).expect("Failed to deserialize embedding request");
